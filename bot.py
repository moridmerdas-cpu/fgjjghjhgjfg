...
async def _handle_command(cl, event, text, owner_id, entry, had_dot=True):
    ...
    # ─── تبچی (مدیریت بنرها) ────────────────────────────────────────────────
        ...
        for slot, b in data.items():
            if b.get("target_mode") and not b.get("active"):
                b["active"] = True
                count += 1
        if count:
            _save_tabchi(owner_id, data)
        await edit(f"تبچی روشن شد؛ {count} بنر به‌صورت خودکار فعال شد و بنرهای فعال طبق مقصدشان ارسال می‌شوند.")
...
def _help_text():
    ...
    sections = [
        ("🔹 اصلی", [
            ...
            "وضعیت",
            "راهنما",
            "پینگ  ← تست سرعت پاسخ‌دهی",
            "ایدی  ← اطلاعات خودت / با ریپلای اطلاعات کاربر",
        ]),
        ("🔹 ابزار", [
            "محاسبه [عبارت]  ← مثال: محاسبه 2+2*3",
            "ترجمه [متن]  ← ترجمه‌ی متن نوشته‌شده",
            "ترجمه متن  ← ریپلای روی پیام برای ترجمه‌اش",
            ".هوا [شهر]",
            "ارز  ← دلار، تتر، یورو، پوند",
            "ارز دلار / ارز تتر / ارز یورو / ارز پوند / ارز بیت کوین",
            "لوگو  ← ساخت بنر تزئینی از عکس پروفایل خودت",
            "اسکرین  ← ریپلای روی پیام؛ ساخت استیکر از پیام+پروفایل فرستنده",
            "اسکرین [لینک پست کانال]  ← اسکرین از یک پست کانال",
            "تبدیل به گیف  ← ریپلای روی ویدیو",
            "دانلود [لینک پست تلگرام]  ← کپی پست به پیام‌های ذخیره‌شده",
            "اینستا [لینک پست یا ریل اینستاگرام]  ← دانلود از اینستاگرام",
        ]),
        ("🔹 گروه", [
            "تگ [متن دلخواه]  ← منشن همه اعضای گروه با متن",
            "تگ [عدد]  ← منشن فقط تا سقف تعداد مشخص‌شده",
            "لغو تگ  ← توقف عملیات تگ در حال اجرا",
            "ترک همگانی گروه  ← خروج از همه‌ی گروه‌ها",
            "ترک همگانی کانال  ← خروج از همه‌ی کانال‌ها",
        ]),
        ("🔹 لیست‌ها", [
        ...
        ("🔹 بلاک کاربر", [
            "بلاک کاربر  ← ریپلای یا آیدی/یوزرنیم",
            "انبلاک کاربر  ← ریپلای یا آیدی/یوزرنیم",
            "لیست بلاک",
            "پاکسازی لیست بلاک  ← آنبلاک همه و خالی‌کردن لیست",
        ]),
        ("🔹 منشی", [
            "منشی روشن",
            "منشی خاموش",
        ...
        ("🔹 امنیت", [
            ...
            "──────────────────",
            "سایلنت چت روشن  ← بی‌صدا شدن نوتیف این چت",
            "سایلنت چت خاموش",
            "سایلنت کاربر [آیدی عددی]",
            "لغو سایلنت کاربر [آیدی عددی]",
            "──────────────────",
            "فیلتر کلمه [کلمه]  ← افزودن کلمه به فیلتر",
            "حذف فیلتر کلمه [کلمه]",
            "لیست فیلتر کلمات",
            "فیلترکلمات روشن  ← حذف خودکار پیام‌های پیوی حاوی کلمات فیلترشده",
            "فیلترکلمات خاموش",
        ]),
        ("🔹 نگهبان چت", [
            "ذخیره پیام حذف‌شده روشن",
            "ذخیره پیام حذف‌شده خاموش",
            "ذخیره پیام ویرایش‌شده روشن",
            "ذخیره پیام ویرایش‌شده خاموش",
            "ذخیره عکس تایمی روشن  ← ذخیره‌ی عکس/ویدیوی تایم‌دار (view once)",
            "ذخیره عکس تایمی خاموش",
        ]),
        ("🔹 پاسخ خودکار و کلیدی", [
            "پاسخ خودکار روشن",
            "پاسخ خودکار خاموش",
            "متن پاسخ خودکار [متن دلخواه]",
            "──────────────────",
            "پاسخ کلیدی [کلمه] = [پاسخ]  ← مثال: پاسخ کلیدی قیمت = قیمت‌ها توی کانال هست.",
            "حذف پاسخ کلیدی [کلمه]",
            "لیست پاسخ کلیدی",
            "پاک کردن پاسخ کلیدی",
        ]),
        ("🔹 هوش مصنوعی", [
            "دیپ سیک روشن  ← پاسخ خودکار هوش مصنوعی به پیوی وقتی غایبی",
            "دیپ سیک خاموش",
            "هوش مصنوعی پاسخ همه روشن  ← پاسخ به همه پیام‌های پیوی، نه فقط وقتی غایبی",
            "هوش مصنوعی پاسخ همه خاموش",
            "آموزش هوش مصنوعی [متن]  ← افزودن اطلاعات به دانش هوش مصنوعی",
        ...
        ("🔹 اکشن ۲۴ ساعته", [
            "تایپینگ روشن / خاموش  ← نمایش «در حال تایپ...» به‌صورت دائم",
            "گیمینگ روشن / خاموش  ← نمایش «در حال بازی...» به‌صورت دائم",
            "ویس روشن / خاموش  ← نمایش «در حال ضبط ویس...» به‌صورت دائم",
            "ارسال ویدیو روشن / خاموش  ← نمایش «در حال ارسال ویدیو...» به‌صورت دائم",
            "💡 این چهار اکشن با هم نمی‌شن؛ روشن‌کردن یکی، بقیه رو خاموش می‌کنه",
        ]),
        ("🔹 اتوماسیون", [
            "سین خودکار روشن",
            "سین خودکار خاموش",
            "ری‌اکشن روشن",
            "ری‌اکشن خاموش",
            "ری‌اکشن [ایموجی]  ← تغییر ایموجی ری‌اکشن عمومی",
            "تنظیم ری‌اکت [ایموجی]  ← ریپلای روی پیام کاربر؛ ری‌اکت اختصاصی فقط برای او",
            "حذف ری‌اکت  ← ریپلای روی پیام کاربر؛ حذف ری‌اکت اختصاصی او",
            "ذخیره مدیا روشن",
            "ذخیره مدیا خاموش",
            "ساعت نام روشن",
            "ساعت نام خاموش",
        ...
        ("🔹 تبچی (مدیریت بنرها)", [
            "تبچی روشن  ← فعال‌سازی خودکار بنرهای دارای مقصد + شروع ارسال چرخشی",
            "تبچی خاموش",
            "تنظیم بنر {عدد}  ← با ریپلای روی پیام بنر — مقصدش خودِ همین گپ می‌شود",
            "تنظیم بنر {عدد} در همه گپ‌ها  ← با ریپلای — مقصدش همه‌ی گروه‌هایت می‌شود",
            "فعال کردن بنر {عدد}",
            "غیرفعال کردن بنر {عدد}",
            "لیست بنر",
            "پاکسازی لیست بنر  ← حذف کامل همه‌ی بنرها",
            "پاکسازی بنر در این چت  ← حذف مقصدِ بنرهای متعلق به همین چت",
            "حذف بنر {عدد}  ← حذف کامل یک بنر",
            "حذف بنر {عدد} در این چت",
            "حذف بنر {عدد} از همه گروه‌ها",
            "حذف بنرها از همه گروه‌ها",
            "فور بنر {عدد} در 50 گروه اخیر  ← ارسال فوری در ۵۰ گروه اخیر",
            "فور بنر {عدد} در 100 گروه اخیر",
            "ارسال بنر برای تمام گروه‌ها  ← ارسال فوریِ همه‌ی بنرهای فعال",
            "تایم بنر {عدد} [دقیقه]  ← فاصله‌ی ارسال خودکار",
            "{عدد} عددی بین ۱ تا ۱۰ است.",
        ]),
        ("🔹 اسپم", [
        ...
        ("🔹 پیام", [
            "ذخیره [1-10]  ← ریپلای",
            "ارسال ذخیره [1-10]",
            "حذف  ← ریپلای روی پیام؛ حذف پیام (و پیام خودت)",
            "حذف بعد [ثانیه]",
    ...
    "tabchi": {
        ...
        "actions": [
            ("راهنما", "PANELINFO::"
             ...
             "پاکسازی لیست بنر\n"
             "ارسال بنر برای تمام گروه‌ها  (ارسال فوریِ همه‌ی بنرهای فعال)\n"
             "تایم بنر {عدد}   (فاصله‌ی ارسال خودکار به دقیقه)\n"
             "{عدد} عددی بین ۱ تا ۱۰ است."
             ),
            ("لیست بنر", "لیست بنر"),
