...
def start_token_bot():
    ...
    if not config.BOT_TOKEN:
        print("BOT_TOKEN تنظیم نشده — ربات الماس غیرفعال است")
        return
    ...
    def send_forced_channels_menu(message, missing_channels):
        ...
        for ch in missing_channels:
            ...
            markup.add(types.InlineKeyboardButton(f"عضویت در {ch}", url=f"https://t.me/{ch_clean}", style="primary"),
                icon_custom_emoji_id=get_emoji_id("megaphone"))
    ...
    def _admin_panel_keyboard():
        ...
        markup.add(
            types.InlineKeyboardButton("تنظیمات خوش‌آمد", callback_data="admin_welcome_settings", style="primary") # 🔵 آبی
        )
        markup.add(
    ...
    def _wc_api_get(endpoint: str) -> dict:
        ...
        if not api_key:
            print("FOOTBALL_API_KEY تنظیم نشده — درخواست به Football API ارسال نشد.")
            return {}
    ...
    def _wc_send_challenge_to_channel(challenge_id: int, team1: str, team2: str, match_time_str: str):
        ...
        if not channel:
            print("WC_CHANNEL_ID تنظیم نشده! چالش جام جهانی به هیچ کانالی ارسال نمی‌شود.")
            return
        ...
        markup.add(
            types.InlineKeyboardButton("مساوی", callback_data=f"wc_pick_{challenge_id}_draw", style="success",
                icon_custom_emoji_id=get_emoji_id("person_check"))
        )
    ...
    if not _wc_channel_cfg:
        print("WC_CHANNEL_ID تنظیم نشده — چالش‌های جام جهانی به هیچ کانالی ارسال نمی‌شوند.")
    else:
    ...
    def _notify_subscription_expired(account_id: int, tg_id: int):
        ...
        try:
            ...
            markup.add(types.InlineKeyboardButton("تمدید اشتراک", callback_data="pur_sub_diamond", style="success"),
                icon_custom_emoji_id=get_emoji_id("inbox"))
            if site_url:
        ...
        try:
            ...
            if data == "self_mgmt_stop":
                ...
                if not bot_manager.is_running(acc_id):
                    _bot.answer_callback_query(call.id, "سلف از قبل خاموش است.", show_alert=True)
                else:
    ...
    def callback_remove_self_ask(call):
        try:
            ...
            if db.get_setting(account["id"], "logged_in", "0") != "1":
                return _bot.answer_callback_query(call.id, "سلف فعالی برای حذف وجود ندارد.", show_alert=True)
    ...
    def cmd_balance(message):
        try:
            ...
            account = _get_account_cached(message.from_user.id)
            if not account:
                return _bot.reply_to(message, "ابتدا در پنل وب ثبت‌نام کنید.", reply_markup=_main_inline_keyboard())
            stats = db.get_token_stats(account["id"])
    ...
    @_bot.message_handler(func=lambda m: m.text and m.text.strip() in ("خرید", "خرید الماس", f"{pe('inbox', '🛒')} خرید"), chat_types=['private'])
    def cmd_buy(message):
        ...
        try:
            ...
            # ── اشتراک با الماس ─────────────────────────────────────────────
                ...
                if balance < cost:
                    ...
                    markup.add(types.InlineKeyboardButton("خرید الماس", callback_data="pur_buy_diamond", style="success"),
                        icon_custom_emoji_id=get_emoji_id("briefcase"))
                    # 🔴 دکمه بازگشت با رنگ danger (قرمز)
            ...
            # ── تأیید/رد پرداخت توسط ادمین ─────────────────────────────────
                ...
                if payment["status"] != "pending":
                    return _bot.answer_callback_query(call.id, "این پرداخت قبلاً پردازش شده", show_alert=True)
        ...
        try:
            if data == "admin_panel" or data == "admin_back":
                ...
                markup.add(types.InlineKeyboardButton("چالش‌های فعال", callback_data="wc_list", style="primary"),
                    icon_custom_emoji_id=get_emoji_id("edit_box"))
                # 🔴 دکمه بازگشت با رنگ danger (قرمز)
                markup.add(types.InlineKeyboardButton("بازگشت", callback_data="admin_panel", style="danger"),
                    icon_custom_emoji_id=get_emoji_id("arrow_blue"))
                _bot.edit_message_text(
                ...
                if not getattr(config, "FOOTBALL_API_KEY", ""):
                    text = "FOOTBALL_API_KEY تنظیم نشده است."
                    markup = types.InlineKeyboardMarkup()
                    # 🔴 دکمه بازگشت با رنگ danger (قرمز)
                    markup.add(types.InlineKeyboardButton("بازگشت", callback_data="admin_panel", style="danger"),
                        icon_custom_emoji_id=get_emoji_id("arrow_blue"))
                ...
                status_fa = {
                    "SCHEDULED": "", "TIMED": f"{pe('timer_1m', '⏳')}",
                    "LIVE": "", "IN_PLAY": f"{pe('exclamation_red', '🔴')}", "PAUSED": "⏸️",
                    "FINISHED": "", "POSTPONED": f"{pe('location', '📌')}",
                ...
                markup.add(
                    ...
                    types.InlineKeyboardButton("پنل یک روزه (1 روز)", callback_data="admin_gift_plan_1", style="primary",
                        icon_custom_emoji_id=get_emoji_id("calendar"))
                )
                markup.add(types.InlineKeyboardButton("لغو", callback_data="admin_panel", style="danger"),
                    icon_custom_emoji_id=get_emoji_id("cross_red"))
                _bot.edit_message_text(
                ...
                markup = types.InlineKeyboardMarkup(row_width=1)
                markup.add(
                    types.InlineKeyboardButton("تغییر متن خوش‌آمد", callback_data="admin_welcome_edit_text", style="primary"),
                ...
                if active:
                    ...
                    for lot in active:
                        status = "فعال" if lot.get("status") == "active" else f"{pe('checkmark_green', '✅')} پایان یافته"
                ...
                markup.add(types.InlineKeyboardButton("مدیریت قرعه‌کشی", callback_data="admin_lottery", style="success"),
                    icon_custom_emoji_id=get_emoji_id("game_controller"))
                markup.add(types.InlineKeyboardButton("پنل مدیریت", callback_data="admin_panel", style="danger"),
                    icon_custom_emoji_id=get_emoji_id("arrow_blue"))
                _bot.edit_message_text(
        ...
        try:
            ...
            # ── قرعه‌کشی states ─────────────────────────────────────────────
                ...
                if current < total:
                    ...
                    markup = types.InlineKeyboardMarkup(row_width=2)
                    markup.add(
                        types.InlineKeyboardButton("تأیید و ثبت", callback_data="lottery_confirm", style="success"),
                        types.InlineKeyboardButton("لغو", callback_data="admin_lottery", style="danger",
                            icon_custom_emoji_id=get_emoji_id("cross_red"))
                    )
                    _bot.reply_to(message,
    ...
    def _subadmin_panel_keyboard():
        ...
        markup.add(
            types.InlineKeyboardButton("شرکت‌کنندگان جام جهانی", callback_data="sa_wc_participants", style="primary",
                icon_custom_emoji_id=get_emoji_id("group_add"))
        )
        markup.add(
    ...
    def _do_missions(tg_id, chat_id):
        try:
            ...
            for m in missions:
                ...
                status = "" if done else f"{pe('timer_1m', '⏳')}"
    ...
    def cmd_hokm_start(message):
        try:
            ...
            if existing:
                return _bot.reply_to(message, "شما در حال حاضر در یک بازی حکم هستید!")
    ...
    def callback_rps_pick(call):
        try:
            ...
            with _rps_lock:
                ...
                if user.id == game["player1"]:
                    if game["choice1"]:
                        return _bot.answer_callback_query(call.id, "شما قبلاً انتخاب کردید.", show_alert=True)
                    game["choice1"] = choice
                    ...
                    if not lot.get("announced") and now_teh >= start_dt:
                        try:
                            ...
                            for i, p in enumerate(prizes):
                                ord_name = ordinals[i] if i < len(ordinals) else f"{i+1}م"
                                medals = ["🥇","🥈","🥉","",f"{pe('trophy', '🏅')}",f"{pe('trophy', '🏅')}",f"{pe('trophy', '🏅')}",f"{pe('trophy', '🏅')}",f"{pe('trophy', '🏅')}",f"{pe('trophy', '🏅')}"]
