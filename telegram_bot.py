...
# نکته: تابع e() با نام «pe» (premium emoji) ایمپورت می‌شه، چون توی این فایل جای
# زیادی از «except Exception as e» استفاده شده و اسم e با اون تداخل پیدا می‌کرد.
...
def _format_plan_remaining(owner_id: int) -> str:
    ...
    if secs <= 0:
        return f"{pe('cross_red', '❌')} منقضی شده"
...
def _kp_markup(digits, mode="code"):
    ...
    markup.add(types.InlineKeyboardButton(f"{pe('cross_red', '❌')} لغو", callback_data="reg_cancel", style="danger"))  # 🔴 قرمز
    return markup
...
def _check_membership_cached(user_id):
    ...
    try:
        ...
        print(f"{pe('warning', '⚠️')} خطا در بررسی عضویت: {e}")
        return True, []
...
def start_token_bot():
    ...
    if not config.BOT_TOKEN:
        print(f"{pe('warning', '⚠️')} BOT_TOKEN تنظیم نشده — ربات الماس غیرفعال است")
        return
    ...
    try:
        ...
        print(f"{pe('robot', '🤖')} ربات الماس: @{BOT_USERNAME}")
    except Exception as e:
        print(f"{pe('cross_red', '❌')} خطا در اتصال ربات الماس: {e}")
        _bot = None
        return
    ...
    def send_forced_channels_menu(message, missing_channels):
        ...
        for ch in missing_channels:
            ...
            markup.add(types.InlineKeyboardButton(f"{pe('megaphone', '📢')} عضویت در {ch}", url=f"https://t.me/{ch_clean}", style="primary"))
        # 🟢 دکمه بررسی با رنگ success (سبز)
        markup.add(types.InlineKeyboardButton(f"{pe('checkmark_green', '✅')} بررسی عضویت من", callback_data="check_join", style="success"))
        
        channels_list = "\n".join([f"{pe('arrow_blue', '🔸')} {ch}" for ch in missing_channels])
        _bot.reply_to(
            message,
    ...
    def _user_keyboard(show_remove_self=True):
        ...
        markup.add(
            types.KeyboardButton(f"{pe('robot', '🤖')} مدیریت سلف", style="primary"),  # 🔵 آبی
        )
        return markup
    ...
    def _owner_keyboard(show_remove_self=True):
        markup = types.ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
        markup.add(
            types.KeyboardButton(f"{pe('megaphone', '📢')} مدیریت", style="danger"),        # 🔴 قرمز
            types.KeyboardButton(f"{pe('robot', '🤖')} مدیریت سلف", style="primary")   # 🔵 آبی
        )
        return markup
    ...
    def _main_inline_keyboard(account=None):
        ...
        markup.add(
            types.InlineKeyboardButton(
                f"{pe('diamond', '💎')} موجودی", callback_data="menu_balance", style="primary",
                icon_custom_emoji_id=get_emoji_id("diamond"),
            ),
            types.InlineKeyboardButton(
                f"{pe('gift', '🎁')} هدیه روزانه", callback_data="menu_daily", style="success",
                icon_custom_emoji_id=get_emoji_id("gift"),
            ),
        )
        markup.add(
            types.InlineKeyboardButton(
                f"{pe('link_chain', '🔗')} رفرال", callback_data="menu_referral", style="primary",
                icon_custom_emoji_id=get_emoji_id("link_chain"),
            ),
            types.InlineKeyboardButton(
                "خرید", callback_data="menu_buy", style="success",
                icon_custom_emoji_id=get_emoji_id("diamond_blue"),
            ),
        )
        markup.add(
            types.InlineKeyboardButton(
                f"{pe('trophy', '🎯')} ماموریت‌ها", callback_data="menu_missions", style="primary",
                icon_custom_emoji_id=get_emoji_id("trophy"),
            )
        )
        markup.add(
            types.InlineKeyboardButton(
                f"{pe('book', '📖')} راهنما", callback_data="guide_menu", style="success",
        ...
        if account is not None:
            ...
            if not is_logged_in:
                markup.add(
                    types.InlineKeyboardButton(
                        f"{pe('robot', '🤖')} ورود سلف با ربات", callback_data="reg_start", style="success",
    ...
    def _admin_panel_keyboard():
        ...
        markup = types.InlineKeyboardMarkup(row_width=2)
        markup.add(
            types.InlineKeyboardButton(f"{pe('megaphone', '📢')} چنل‌های اجباری", callback_data="admin_channels", style="primary"),   # 🔵 آبی
            types.InlineKeyboardButton(f"{pe('group_add', '👥')} کاربران", callback_data="admin_users", style="primary")              # 🔵 آبی
        )
        markup.add(
            types.InlineKeyboardButton(f"{pe('trophy', '🏆')} جام جهانی", callback_data="admin_wc", style="success"),              # 🟢 سبز
            types.InlineKeyboardButton(f"{pe('calendar', '📅')} بازی‌های امروز", callback_data="admin_today_games", style="primary") # 🔵 آبی
        )
        markup.add(
            types.InlineKeyboardButton(f"{pe('diamond', '💎')} انتقال الماس", callback_data="admin_transfer", style="primary"),    # 🔵 آبی
            types.InlineKeyboardButton(f"{pe('diamond', '💰')} دادن الماس", callback_data="admin_give", style="success")           # 🟢 سبز
        )
        markup.add(
        ...
        markup.add(
            types.InlineKeyboardButton(f"{pe('megaphone', '📣')} پیام عمومی", callback_data="admin_broadcast", style="primary"),      # 🔵 آبی
            types.InlineKeyboardButton(f"{pe('trophy', '🎯')} ماموریت‌ها", callback_data="admin_missions", style="success")       # 🟢 سبز
        )
        markup.add(
            types.InlineKeyboardButton(f"{pe('group_add', '👥')} شرکت‌کنندگان جام جهانی", callback_data="admin_wc_participants", style="primary") # 🔵 آبی
        )
        markup.add(
            types.InlineKeyboardButton(f"{pe('gift', '🎁')} هدیه", callback_data="admin_gift", style="success")                 # 🟢 سبز
        )
        markup.add(
            types.InlineKeyboardButton(f"{pe('shield', '👮')} مدیریت ادمین‌ها", callback_data="admin_manage_admins", style="primary") # 🔵 آبی
        )
        markup.add(
            types.InlineKeyboardButton(f"{pe('book', '📚')} مدیریت راهنما", callback_data="admin_guide_manage", style="success")    # 🟢 سبز
        )
        markup.add(
            types.InlineKeyboardButton(f"{pe('pencil', '✏️')} تنظیمات خوش‌آمد", callback_data="admin_welcome_settings", style="primary") # 🔵 آبی
        )
        markup.add(
            types.InlineKeyboardButton(f"{pe('game_controller', '🎰')} قرعه‌کشی", callback_data="admin_lottery", style="success")           # 🟢 سبز
        )
        markup.add(
            types.InlineKeyboardButton(f"{pe('arrow_blue', '🔙')} بازگشت", callback_data="admin_panel", style="danger")               # 🔴 قرمز
        )
        return markup
    ...
    def cmd_bet(message):
        try:
            ...
            if len(parts) < 3:
                return _bot.reply_to(message, f"{pe('exclamation_red', '❗')} فرمت: شرط بندی [مقدار]\nمثال: شرط بندی 100")

            try:
                amount = int(parts[2])
                if amount < 1:
                    return _bot.reply_to(message, f"{pe('cross_red', '❌')} مقدار باید بیشتر از ۰ باشد.")
            except ValueError:
                return _bot.reply_to(message, f"{pe('cross_red', '❌')} مقدار باید عدد باشد.")

            account = _get_account_cached(message.from_user.id)
            if not account:
                return _bot.reply_to(message, f"{pe('warning', '⚠️')} ابتدا در پنل وب ثبت‌نام کنید.")

            balance = db.get_token_balance(account["id"])
            if balance < amount:
                return _bot.reply_to(
                    message,
                    f"{pe('cross_red', '❌')} موجودی کافی ندارید!\nنیاز: {amount} الماس — موجودی: {balance} الماس"
                )

            bet_id = db.create_bet(account["id"], message.from_user.id, amount, message.chat.id)
            if not bet_id:
                return _bot.reply_to(message, f"{pe('cross_red', '❌')} خطا در ساخت شرط‌بندی. دوباره امتحان کنید.")
            ...
            msg = _bot.reply_to(
                message,
                f"{pe('game_controller', '🎲')} <b>شرط‌بندی باز شد!</b>\n\n"
                f"{pe('person_star', '👤')} سازنده: {creator_name}\n"
                f"{pe('diamond', '💎')} مبلغ: <b>{amount} الماس</b>\n"
                f"{pe('trophy', '🏆')} جایزه برنده: <b>{payout} الماس</b> (بعد از ۱۷٪ مالیات)\n\n"
                f"{pe('timer_1m', '⏳')} منتظر حریف...\n"
            ...
            print(f"{pe('cross_red', '❌')} خطا در cmd_bet: {e}")
            _bot.reply_to(message, f"{pe('cross_red', '❌')} خطا: {e}")
    ...
    def callback_join_bet(call):
        try:
            ...
            if bet_mem is None:
                return _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} این شرط‌بندی یافت نشد یا منقضی شده.", show_alert=True)

            if bet_mem["opponent_tg_id"] is not None:
                return _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} این شرط‌بندی قبلاً تکمیل شده است.", show_alert=True)

            if bet_mem["creator_tg_id"] == call.from_user.id:
                return _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} شما سازنده این شرط هستید! منتظر حریف باشید.", show_alert=True)

            account = _get_account_cached(call.from_user.id)
            if not account:
                return _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} ابتدا در پنل وب ثبت‌نام کنید.", show_alert=True)
            ...
            _bot.answer_callback_query(call.id, f"{pe('checkmark_green', '✅')} وارد شرط‌بندی شدید! بازی شروع می‌شود...", show_alert=True)
            ...
            result_text = (
                f"{pe('gift', '🎉')} <b>شرط‌بندی به پایان رسید!</b>\n\n"
                f"⚔️ حریف: {opponent_name}\n"
                f"{pe('diamond', '💎')} مبلغ هر نفر: {amount} الماس\n"
                f"{pe('diamond', '💰')} مجموع: {total} الماس\n"
                f"🏛 مالیات (۱۷٪): {tax} الماس\n\n"
                f"{pe('trophy', '🏆')} <b>برنده: {winner_name}</b>\n"
                f"{pe('diamond', '💎')} <b>جایزه: {payout} الماس</b>"
            )
            ...
            try:
                _bot.send_message(
                    loser_tg_id,
                    f"😔 متأسفانه این بار نبردید.\n{pe('diamond', '💎')} {amount} الماس از حسابتان کسر شد."
                )
            except Exception:
                pass
            ...
            print(f"{pe('cross_red', '❌')} خطا در callback_join_bet: {e}")
            try:
                _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} خطا: {str(e)[:100]}", show_alert=True)
            except Exception:
                pass
    ...
    def callback_cancel_bet(call):
        try:
            ...
            bet_mem = _active_bets.get(bet_id)
            if bet_mem is None:
                return _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} این شرط‌بندی یافت نشد یا قبلاً لغو شده.", show_alert=True)

            if bet_mem["opponent_tg_id"] is not None:
                return _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} حریف وارد شده — دیگر نمی‌توانید لغو کنید.", show_alert=True)

            if bet_mem["creator_tg_id"] != call.from_user.id:
                return _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} فقط سازنده شرط می‌تواند لغو کند.", show_alert=True)
            ...
            _bot.answer_callback_query(call.id, f"{pe('checkmark_green', '✅')} شرط‌بندی لغو شد و مبلغ بازگشت داده شد.", show_alert=True)

        except Exception as e:
            print(f"{pe('cross_red', '❌')} خطا در callback_cancel_bet: {e}")
            try:
                _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} خطا: {str(e)[:100]}", show_alert=True)
            except Exception:
                pass
    ...
    def _auto_cancel_bet(bet_id, chat_id, message_id):
        try:
            ...
            try:
                _bot.edit_message_text(
                    f"{pe('timer_1h', '⏰')} <b>شرط‌بندی لغو شد!</b>\n\nهیچ حریفی وارد نشد.\n{pe('diamond', '💎')} مبلغ به سازنده بازگشت داده شد.",
                    chat_id=chat_id,
                    message_id=message_id
                )
            except Exception:
                _bot.send_message(chat_id, f"{pe('timer_1h', '⏰')} یک شرط‌بندی به دلیل نبود حریف لغو شد.")
        except Exception as e:
            print(f"{pe('cross_red', '❌')} خطا در _auto_cancel_bet: {e}")
    ...
    def cmd_balance_group(message):
        try:
            ...
            _bot.reply_to(
                message,
                f"{pe('diamond', '💎')} <b>موجودی شما:</b>\n\n"
                f"{pe('diamond', '💰')} الماس: <b>{stats['balance']}</b>\n"
                f"{pe('lines', '📊')} کل دریافتی: <b>{stats['total_earned']}</b>"
            )
        except Exception as e:
            print(f"{pe('cross_red', '❌')} خطا در cmd_balance_group: {e}")
        ...
        try:
            ...
            if len(parts) == 2 and message.reply_to_message:
                ...
                if not target_user or target_user.is_bot:
                    return _bot.reply_to(message, f"{pe('cross_red', '❌')} نمی‌توان به این کاربر الماس انتقال داد.")

                try:
                    amount = int(parts[1])
                    if amount < 1:
                        return _bot.reply_to(message, f"{pe('cross_red', '❌')} مقدار باید بیشتر از 0 باشد.")
                except ValueError:
                    return _bot.reply_to(message, f"{pe('cross_red', '❌')} مقدار باید عدد باشد.")

                if target_user.id == message.from_user.id:
                    return _bot.reply_to(message, f"{pe('cross_red', '❌')} نمی‌توانید به خودتان الماس انتقال دهید.")
                ...
                if not to_account:
                    return _bot.reply_to(message, f"{pe('cross_red', '❌')} این کاربر در پنل وب ثبت‌نام نکرده است.")
                ...
                if success:
                    ...
                    if to_tg_id:
                        try:
                            _bot.send_message(
                                to_tg_id,
                                f"{pe('diamond', '💎')} <b>{amount} الماس</b> از @{message.from_user.username or 'کاربر'} دریافت کردید!"
                            )
                        except Exception:
                            pass
            ...
            if len(parts) < 3:
                return _bot.reply_to(message, f"{pe('exclamation_red', '❗')} فرمت: انتقال [یوزرنیم] [تعداد]\nمثال: انتقال @ali 10\nیا روی پیام کاربر ریپلای کنید و بنویسید: انتقال [تعداد]")
            ...
            if not to_account:
                return _bot.reply_to(message, f"{pe('cross_red', '❌')} کاربر '{username}' یافت نشد.")
            ...
            print(f"{pe('cross_red', '❌')} خطا در cmd_transfer: {e}")
            _bot.reply_to(message, f"{pe('cross_red', '❌')} خطا: {e}")
    ...
    def _wc_api_get(endpoint: str) -> dict:
        ...
        if not api_key:
            print(f"{pe('warning', '⚠️')} FOOTBALL_API_KEY تنظیم نشده — درخواست به Football API ارسال نشد.")
            return {}
        ...
        try:
            ...
            try:
                ...
                pass
            print(f"{pe('cross_red', '❌')} Football API HTTP {e.code} [{endpoint}]: {body}")
            return {}
        except Exception as e:
            print(f"{pe('cross_red', '❌')} Football API error [{endpoint}]: {e}")
            return {}
    ...
    def _wc_send_challenge_to_channel(challenge_id: int, team1: str, team2: str, match_time_str: str):
        ...
        if not channel:
            print(f"{pe('warning', '⚠️')} WC_CHANNEL_ID تنظیم نشده! چالش جام جهانی به هیچ کانالی ارسال نمی‌شود.")
            return
        ...
        markup.add(
            types.InlineKeyboardButton(f"{pe('person_check', '🤝')} مساوی", callback_data=f"wc_pick_{challenge_id}_draw", style="success")
        )
        # 🔴 دکمه تیم دوم با رنگ danger (قرمز)
        markup.add(
            types.InlineKeyboardButton(f"{pe('exclamation_red', '🔴')} {team2}", callback_data=f"wc_pick_{challenge_id}_team2", style="danger")
        )
        now_tehran = _now_tehran().strftime("%Y/%m/%d — %H:%M")
        text = (
            f"⚽️ <b>چالش جام جهانی ۲۰۲۶</b>\n\n"
            f"🆚 <b>{team1}</b>  vs  <b>{team2}</b>\n"
            f"{pe('timer_1h', '⏰')} زمان بازی: <b>{match_time_str}</b>\n"
            f"{pe('timer_1h', '🕐')} ارسال در: {now_tehran} (تهران)\n\n"
            f"{pe('diamond', '💎')} محدوده شرط: {min_bet:,} – {max_bet:,} الماس\n"
            f"{pe('trophy', '🏆')} برندگان ۲ برابر مبلغ شرط دریافت می‌کنند!\n\n"
        ...
        try:
            ...
            print(f"{pe('checkmark_green', '✅')} چالش به کانال {chat_target} ارسال شد (msg_id={msg.message_id})")
        except Exception as e:
            print(f"{pe('cross_red', '❌')} ارسال چالش به کانال {chat_target} ناموفق بود: {e}\n"
        ...
        try:
            ...
            for m in matches:
                ...
                if challenge_id:
                    ...
                    print(f"{pe('checkmark_green', '✅')} چالش جدید ساخته شد: {home} vs {away} (ID: {challenge_id})")
                    time.sleep(0.3)  # جلوگیری از flood در ارسال به کانال
        except Exception as e:
            print(f"{pe('cross_red', '❌')} _wc_auto_fetch_and_create: {e}")
        ...
        try:
            ...
            for ch in pending:
                ...
                result_text = (
                    ...
                    f"{pe('trophy', '🏆')} نتیجه: <b>{option_fa}</b>\n\n"
                    f"{pe('checkmark_green', '✅')} برندگان ۲ برابر مبلغ شرطشان دریافت کردند!"
                )
                ...
                for winner in paid:
                    try:
                        _bot.send_message(
                            winner["user_tg_id"],
                            f"{pe('gift', '🎉')} <b>تبریک!</b> شرط‌بندی {ch['team1']} vs {ch['team2']} را بردید!\n"
                            f"{pe('diamond', '💎')} <b>{winner['payout']} الماس</b> به حسابتان واریز شد."
                        )
                    except Exception:
                        pass
        except Exception as e:
            print(f"{pe('cross_red', '❌')} _wc_auto_check_results: {e}")
        ...
        while True:
            try:
                ...
                for m in matches:
                    ...
                    if challenge_id:
                        _wc_send_challenge_to_channel(challenge_id, home, away, match_time_str)
                        print(f"{pe('checkmark_green', '✅')} چالش ۱ ساعت قبل ارسال شد: {home} vs {away}")
                    time.sleep(0.5)
                ...
                print(f"{pe('cross_red', '❌')} _wc_scheduler: {e}")
            time.sleep(POLL)
    ...
    if not _wc_channel_cfg:
        print(f"{pe('warning', '⚠️')} WC_CHANNEL_ID تنظیم نشده — چالش‌های جام جهانی به هیچ کانالی ارسال نمی‌شوند.")
    else:
        ...
        try:
            ...
            if member.status not in ("administrator", "creator"):
                print(f"{pe('warning', '⚠️')} ربات در کانال {_wc_target} ادمین نیست — ارسال پیام به کانال شکست خواهد خورد.")
            else:
                print(f"{pe('checkmark_green', '✅')} دسترسی به کانال جام جهانی تأیید شد: {getattr(chat_info, 'title', _wc_target)}")
        except Exception as e:
            print(f"{pe('cross_red', '❌')} ربات نتوانست به کانال {_wc_target} دسترسی پیدا کند: {e}\n"
                  f"   بررسی کنید ربات در کانال عضو/ادمین باشد و WC_CHANNEL_ID صحیح باشد.")
    if not getattr(config, "FOOTBALL_API_KEY", ""):
        print(f"{pe('warning', '⚠️')} FOOTBALL_API_KEY تنظیم نشده — هیچ بازی‌ای از Football API دریافت نمی‌شود.")
    ...
    def callback_wc_pick(call):
        try:
            ...
            if not challenge or challenge["status"] != "pending":
                return _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} این چالش دیگر فعال نیست.", show_alert=True)
            ...
            _bot.answer_callback_query(call.id, f"{pe('checkmark_green', '✅')} انتخاب: {option_fa}", show_alert=False)
            try:
                _bot.send_message(
                    call.from_user.id,
                    f"⚽️ انتخاب شما: <b>{option_fa}</b>\n\n"
                    f"{pe('diamond', '💎')} مبلغ شرط را وارد کنید ({min_bet} تا {max_bet} الماس):\n"
                ...
                _bot.answer_callback_query(
                    call.id,
                    f"{pe('checkmark_green', '✅')} انتخاب: {option_fa}\n\n"
                    f"برای ثبت شرط، به ربات پیام بده:\nشرکت [مبلغ]\nمثال: شرکت 200",
                    show_alert=True
                )
        except Exception as e:
            print(f"{pe('cross_red', '❌')} callback_wc_pick: {e}")
    ...
    def cmd_wc_join(message):
        try:
            ...
            if not pending:
                return _bot.reply_to(message, f"{pe('cross_red', '❌')} ابتدا روی تیم مورد نظر در کانال کلیک کنید.")

            parts = message.text.strip().split()
            if len(parts) < 2:
                return _bot.reply_to(message, f"{pe('cross_red', '❌')} فرمت: شرکت [مبلغ]\nمثال: شرکت 200")
            try:
                amount = int(parts[1])
            except ValueError:
                return _bot.reply_to(message, f"{pe('cross_red', '❌')} مبلغ باید عدد باشد.")

            min_bet = getattr(config, "WC_MIN_BET", 10)
            max_bet = getattr(config, "WC_MAX_BET", 5000)
            if amount < min_bet or amount > max_bet:
                return _bot.reply_to(message, f"{pe('cross_red', '❌')} مبلغ باید بین {min_bet} و {max_bet} الماس باشد.")
            ...
            if not challenge:
                ...
                return _bot.reply_to(message, f"{pe('cross_red', '❌')} چالش یافت نشد.")
            ...
            if success:
                ...
                _bot.reply_to(
                    message,
                    f"{pe('checkmark_green', '✅')} <b>شرط ثبت شد!</b>\n\n"
                    f"⚽️ {challenge['team1']} vs {challenge['team2']}\n"
                    f"{pe('trophy', '🎯')} انتخاب: <b>{option_fa}</b>\n"
                    f"{pe('diamond', '💎')} مبلغ: <b>{amount} الماس</b>\n"
                    f"{pe('diamond', '💰')} موجودی باقی‌مانده: {balance} الماس\n\n"
                    f"{pe('trophy', '🏆')} در صورت برد، <b>{amount * 2} الماس</b> دریافت می‌کنید!"
                )
            else:
                _bot.reply_to(message, msg_txt)
        except Exception as e:
            print(f"{pe('cross_red', '❌')} cmd_wc_join: {e}")
            _bot.reply_to(message, f"{pe('cross_red', '❌')} خطا: {e}")
    ...
    def callback_bet_wc(call):
        try:
            ...
            challenge = db.get_wc_challenge(challenge_id)
            if not challenge or challenge["status"] != "pending":
                return _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} این چالش فعال نیست.", show_alert=True)
            account = _get_account_cached(call.from_user.id)
            if not account:
                return _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} ابتدا در پنل وب ثبت‌نام کنید.", show_alert=True)
            _wc_pending_bet[call.from_user.id] = {
            ...
            _bot.answer_callback_query(call.id, f"{pe('checkmark_green', '✅')} انتخاب ثبت شد! حالا مبلغ رو بنویس:\nشرکت [مبلغ]", show_alert=True)
        except Exception as e:
            print(f"{pe('cross_red', '❌')} خطا در callback_bet_wc: {e}")
        ...
        try:
            ...
            if expires:
                ...
                try:
                    _bot.send_message(
                        tg_id,
                        f"{pe('gift', '🎁')} <b>یک روز سلف رایگان هدیه گرفتید!</b>\n\n"
                        f"{pe('timer_1h', '⏰')} انقضا: <b>{exp_str}</b> (وقت تهران)\n\n"
            ...
            print(f"{pe('cross_red', '❌')} _grant_free_trial: {e}")
    ...
    def _notify_subscription_expired(account_id: int, tg_id: int):
        ...
        try:
            ...
            markup.add(types.InlineKeyboardButton(f"{pe('inbox', '🛒')} تمدید اشتراک", callback_data="pur_sub_diamond", style="success"))
            if site_url:
                # 🔵 دکمه وب‌سایت با رنگ primary (آبی)
                markup.add(types.InlineKeyboardButton(f"{pe('link_chain', '🌐')} پنل وب", url=site_url, style="primary"))
            try:
                _bot.send_message(
                    tg_id,
                    f"{pe('timer_1h', '⏰')} <b>اشتراک سلف شما به پایان رسید!</b>\n\n"
            ...
            print(f"{pe('cross_red', '❌')} _notify_subscription_expired: {e}")
        ...
        def _checker():
            while True:
                try:
                    ...
                    print(f"{pe('cross_red', '❌')} subscription checker: {e}")
        ...
        try:
            ...
            for row in (rows or []):
                ...
                try:
                    ...
                    _bot.send_message(
                        tg_id,
                        f"{pe('warning', '⚠️')} <b>اشتراک شما در حال انقضاست!</b>\n\n"
                        f"{pe('timer_1h', '⏰')} باقی‌مانده: <b>{remaining}</b>\n\n"
            ...
            print(f"{pe('cross_red', '❌')} _check_expiring_subscriptions: {e}")
    ...
    def callback_reg_start(call):
        ...
        try:
            ...
            _bot.edit_message_text(
                f"{pe('phone', '📱')} <b>مرحله ۱ از ۳ — شماره تلفن</b>\n\n"
                ...
                reply_markup=types.InlineKeyboardMarkup().add(
                    types.InlineKeyboardButton(f"{pe('cross_red', '❌')} لغو", callback_data="reg_cancel", style="danger")  # 🔴 قرمز
                ),
            )
        except Exception:
            pass
    ...
    def handle_reg_phone(message):
        ...
        wait_msg = _bot.reply_to(message, f"{pe('timer_1m', '⏳')} در حال ارسال کد تأیید...")
        ...
        try:
            ...
            try:
                ...
                pass
            _bot.reply_to(message, f"{pe('cross_red', '❌')} خطا در ارسال کد: {str(e)}\n\nدوباره /start بزنید.")
    ...
    def handle_reg_2fa_text(message):
        ...
        if not password:
            _bot.reply_to(message, f"{pe('exclamation_red', '❗')} رمز نمی‌تواند خالی باشد. دوباره تایپ کنید:")
            return
        ...
        wait_msg = _bot.send_message(tg_id, f"{pe('timer_1m', '⏳')} در حال تأیید رمز دو مرحله‌ای...")
        ...
        try:
            ...
            sent = _bot.send_message(
                tg_id,
                f"{pe('checkmark_green', '✅')} رمز دو مرحله‌ای تأیید شد!\n\n"
                f"{pe('key', '🔑')} <b>مرحله ۳ — رمز عبور پنل</b>\n\n"
            ...
            _bot.send_message(
                tg_id,
                f"{pe('cross_red', '❌')} رمز دو مرحله‌ای اشتباه است!\n\nدوباره رمز را تایپ کنید و بفرستید:",
            )
    ...
    def callback_reg_kp(call):
        ...
        if _reg_expired(tg_id):
            _reg_clear(tg_id)
            _bot.answer_callback_query(call.id, f"{pe('timer_1h', '⏰')} سشن منقضی شده! دوباره /start بزنید.", show_alert=True)
            try:
                _bot.edit_message_text(f"{pe('timer_1h', '⏰')} سشن منقضی شد.", chat_id=call.message.chat.id, message_id=call.message.message_id)
            except Exception:
                pass
            return
        ...
        if action == "del":
            ...
            if len(digits) >= 10:
                _bot.answer_callback_query(call.id, f"{pe('exclamation_red', '❗')} حداکثر ۱۰ رقم", show_alert=True)
                return
            digits += action
        else:
            _bot.answer_callback_query(call.id)
    ...
    def _process_reg_confirm(call, tg_id, session, mode, digits):
        ...
        if not digits:
            _bot.answer_callback_query(call.id, f"{pe('exclamation_red', '❗')} چیزی وارد نکردید!", show_alert=True)
            return

        _bot.answer_callback_query(call.id, f"{pe('timer_1m', '⏳')} در حال بررسی...")
            ...
            try:
                ...
                try:
                    ...
                    if "SessionPasswordNeeded" in err_str or "password" in err_str.lower():
                        ...
                        try:
                            _bot.edit_message_text(
                                f"{pe('padlock', '🔒')} <b>رمز دو مرحله‌ای</b>\n\n"
                        ...
                        try:
                            _bot.edit_message_text(
                                f"{pe('cross_red', '❌')} کد اشتباه بود! دوباره وارد کنید:\n\n"
                                f"<code>{_kp_display('', 'code')}</code>",
                                chat_id=call.message.chat.id,
                                message_id=call.message.message_id,
                                reply_markup=_kp_markup("", "code"),
                        ...
                        try:
                            _bot.edit_message_text(
                                f"{pe('timer_1h', '⏰')} کد منقضی شده! دوباره /start بزنید.",
                                chat_id=call.message.chat.id,
                                message_id=call.message.message_id,
                            )
                        except Exception:
                        ...
                        try:
                            _bot.edit_message_text(
                                f"{pe('cross_red', '❌')} خطا: {err_str[:200]}\n\nدوباره /start بزنید.",
                                chat_id=call.message.chat.id,
                                message_id=call.message.message_id,
                            )
                        except Exception:
                ...
                _reg_clear(tg_id)
                try:
                    _bot.edit_message_text(f"{pe('cross_red', '❌')} خطای داخلی: {str(e)[:200]}", chat_id=call.message.chat.id, message_id=call.message.message_id)
                except Exception:
                    pass
        ...
        # ── تأیید ۲FA ────────────────────────────────────────────────────────
            ...
            try:
                ...
                try:
                    ...
                    try:
                        _bot.edit_message_text(
                            f"{pe('checkmark_green', '✅')} ورود موفق!\n\n"
                            f"{pe('key', '🔑')} <b>مرحله ۳ — رمز عبور پنل</b>\n\n"
                            "یک رمز عبور برای ورود به پنل وب انتخاب کنید:\n"
                            "(حداقل ۴ رقم)\n\n"
                            f"<code>{_kp_display('', 'pw')}</code>",
                ...
                _reg_clear(tg_id)
                try:
                    _bot.edit_message_text(f"{pe('cross_red', '❌')} خطا: {str(e)[:200]}", chat_id=call.message.chat.id, message_id=call.message.message_id)
                except Exception:
                    pass
        ...
        # ── ثبت رمز عبور پنل و ساخت اکانت ──────────────────────────────────
            ...
            if len(digits) < 4:
                _bot.answer_callback_query(call.id, f"{pe('exclamation_red', '❗')} رمز باید حداقل ۴ رقم باشد!", show_alert=True)
                return
            ...
            try:
                ...
                if existing:
                    ...
                    def _start_existing(_acc_id):
                        ...
                        try:
                            ...
                            print(f"{pe('warning', '⚠️')} bot_manager.start (existing): {_e}")
                    threading.Thread(target=_start_existing, args=(existing["id"],), daemon=True).start()

                    try:
                        _bot.edit_message_text(
                            f"{pe('checkmark_green', '✅')} <b>خوش برگشتید!</b>\n\n"
                            f"{pe('person_star', '👤')} {tg_user['name']}\n"
                            f"🆔 اکانت موجود بود — سلف‌بات فعال شد!\n\n"
                            f"{pe('diamond', '💎')} موجودی: <b>{db.get_token_balance(existing['id'])}</b> الماس",
                            chat_id=call.message.chat.id,
                            message_id=call.message.message_id,
                        )
                    except Exception:
                ...
                if not new_id:
                    _reg_clear(tg_id)
                    try:
                        _bot.edit_message_text(f"{pe('cross_red', '❌')} خطا در ساخت اکانت. دوباره /start بزنید.", chat_id=call.message.chat.id, message_id=call.message.message_id)
                    except Exception:
                        pass
                    return
                ...
                try:
                    ...
                    print(f"{pe('warning', '⚠️')} set free_trial on register: {_e}")
                ...
                def _start_new(_acc_id, _tg_id):
                    ...
                    try:
                        ...
                        print(f"{pe('warning', '⚠️')} bot_manager.start (new): {_e}")
                ...
                if site_url:
                    ...
                    markup_done.add(types.InlineKeyboardButton(f"{pe('link_chain', '🌐')} ورود به پنل وب", url=site_url, style="primary"))

                try:
                    _bot.edit_message_text(
                        f"{pe('gift', '🎉')} <b>اکانت ساخته شد!</b>\n\n"
                        f"{pe('person_star', '👤')} نام: <b>{tg_user['name']}</b>\n"
                        f"{pe('key', '🔑')} یوزرنیم پنل: <code>{candidate}</code>\n"
                        f"{pe('padlock', '🔒')} رمز عبور: همان رمزی که وارد کردید\n\n"
                        f"{pe('gift', '🎁')} <b>{config.WELCOME_TOKENS} الماس</b> هدیه خوش‌آمد دریافت کردید!\n"
                        f"{pe('timer_1h', '⏰')} <b>۱ روز سلف رایگان</b> فعال شد!\n\n"
                        f"{pe('checkmark_green', '✅')} سلف‌بات در حال اتصال است — چند لحظه صبر کنید.",
                        chat_id=call.message.chat.id,
                        message_id=call.message.message_id,
    ...
    def _self_management_keyboard(account_id):
        ...
        if not is_logged:
            ...
            markup.add(types.InlineKeyboardButton(
                f"{pe('link_chain', '🔗')} وصل کردن سلف", callback_data="reg_start", style="success"))
        else:
            if is_running and not is_paused:
                # سلف روشن است — دکمه خاموش کردن
                markup.add(types.InlineKeyboardButton(
                    f"{pe('exclamation_red', '🔴')} خاموش کردن سلف", callback_data="self_mgmt_stop", style="danger"))
            else:
                # سلف خاموش یا pause است — دکمه روشن کردن
                markup.add(types.InlineKeyboardButton(
                    f"{pe('checkmark_green', '🟢')} روشن کردن سلف", callback_data="self_mgmt_start", style="success"))
            # حذف سلف همیشه نمایش داده می‌شود
            markup.add(types.InlineKeyboardButton(
                "🗑 حذف سلف از اکانت تلگرام", callback_data="remove_self_ask", style="danger"))

        markup.add(types.InlineKeyboardButton(
            f"{pe('arrow_blue', '🔙')} بازگشت", callback_data="self_mgmt_back", style="danger"))
        return markup
        ...
        if sub:
            ...
            if exp:
                ...
                if diff.total_seconds() > 0:
                    ...
                    sub_line = f"{pe('checkmark_green', '✅')} فعال — باقی‌مانده: <b>{remaining}</b>"
                else:
                    sub_line = f"{pe('cross_red', '❌')} منقضی شده"
            else:
                sub_line = "❓ نامشخص"
        else:
            sub_line = f"{pe('cross_red', '❌')} اشتراک ندارید"

        return (
            f"{pe('robot', '🤖')} <b>مدیریت سلف</b>\n\n"
            f"{status_icon} وضعیت: <b>{status_text}</b>\n"
            f"{pe('briefcase', '📦')} اشتراک: {sub_line}\n\n"
            f"از دکمه‌های زیر استفاده کنید:"
        )

    @_bot.message_handler(func=lambda m: m.text == f"{pe('robot', '🤖')} مدیریت سلف", chat_types=['private'])
    def cmd_self_management(message):
        try:
            ...
            print(f"{pe('cross_red', '❌')} خطا در cmd_self_management: {e}")
    ...
    def callback_self_management(call):
        try:
            account = _get_account_cached(call.from_user.id)
            if not account:
                return _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} اکانت یافت نشد.", show_alert=True)
            ...
            if data == "self_mgmt_stop":
                ...
                if not bot_manager.is_running(acc_id):
                    _bot.answer_callback_query(call.id, f"{pe('warning', '⚠️')} سلف از قبل خاموش است.", show_alert=True)
                else:
                    bot_manager.stop(acc_id)
                    # صبر کوتاه تا state بروز شود
                    _time.sleep(0.8)
                    _bot.answer_callback_query(call.id, f"{pe('exclamation_red', '🔴')} سلف خاموش شد.")
                ...
                if not db.get_setting(acc_id, "logged_in", "0") == "1":
                    return _bot.answer_callback_query(
                        call.id, f"{pe('cross_red', '❌')} سلف وصل نیست. ابتدا از «وصل کردن سلف» استفاده کنید.", show_alert=True)
                if not db.is_subscribed(acc_id):
                    return _bot.answer_callback_query(
                        call.id, f"{pe('cross_red', '❌')} اشتراک ندارید یا منقضی شده. ابتدا پلن تهیه کنید.", show_alert=True)
                if bot_manager.is_running(acc_id) and not bot_manager.is_paused(acc_id):
                    _bot.answer_callback_query(call.id, f"{pe('checkmark_green', '✅')} سلف از قبل روشن است.", show_alert=True)
                else:
                    bot_manager.start(acc_id, get_loop(), check_tokens=False, is_restart=True)
                    # صبر کوتاه تا heartbeat ثبت شود
                    _time.sleep(1.2)
                    _bot.answer_callback_query(call.id, f"{pe('checkmark_green', '🟢')} سلف روشن شد!")
            ...
            print(f"{pe('cross_red', '❌')} خطا در callback_self_management: {e}")
            try:
                _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} خطا: {str(e)[:80]}", show_alert=True)
            except Exception:
                pass
    ...
    def _logout_telegram_session(session_data):
        ...
        try:
            ...
            print(f"{pe('warning', '⚠️')} خطا در خروج سشن تلگرام: {e}")
    ...
    def _remove_self_from_account(account_id: int):
        ...
        try:
            ...
            print(f"{pe('warning', '⚠️')} خطا در توقف سلف: {e}")
    ...
    def callback_remove_self_ask(call):
        try:
            ...
            if db.get_setting(account["id"], "logged_in", "0") != "1":
                return _bot.answer_callback_query(call.id, f"{pe('warning', '⚠️')} سلف فعالی برای حذف وجود ندارد.", show_alert=True)

            markup = types.InlineKeyboardMarkup(row_width=2)
            # 🟢 دکمه تأیید با رنگ success (سبز)
            markup.add(
                types.InlineKeyboardButton(f"{pe('checkmark_green', '✅')} بله، حذف کن", callback_data="remove_self_yes", style="success"),
                types.InlineKeyboardButton(f"{pe('cross_red', '❌')} انصراف", callback_data="remove_self_no", style="danger")  # 🔴 قرمز
            )
            _bot.answer_callback_query(call.id)
            try:
                _bot.edit_message_text(
                    f"{pe('warning', '⚠️')} <b>مطمئن هستید؟</b>\n\n"
                    "با تأیید، سلف از اکانت تلگرامی که الان به آن وصل است خارج می‌شود.\n"
                    f"{pe('diamond', '💎')} الماس‌ها و یوزرنیم پنل شما <b>حفظ می‌شوند</b>.\n\n"
            ...
            print(f"{pe('cross_red', '❌')} خطا در callback_remove_self_ask: {e}")
    ...
    def callback_remove_self_no(call):
        ...
        try:
            _bot.edit_message_text(f"{pe('cross_red', '❌')} عملیات لغو شد.", chat_id=call.message.chat.id, message_id=call.message.message_id)
        except Exception:
            pass
    ...
    def callback_remove_self_yes(call):
        try:
            ...
            _bot.answer_callback_query(call.id, f"{pe('timer_1m', '⏳')} در حال خروج سلف...")
            try:
                _bot.edit_message_text(f"{pe('timer_1m', '⏳')} در حال خروج سلف از اکانت تلگرام...", chat_id=call.message.chat.id, message_id=call.message.message_id)
            except Exception:
                pass
            ...
            try:
                _bot.send_message(
                    call.message.chat.id,
                    f"{pe('checkmark_green', '✅')} <b>سلف با موفقیت از اکانت تلگرام خارج شد.</b>\n\n"
                    f"{pe('person_star', '👤')} یوزرنیم پنل شما (<b>{account['username']}</b>) و موجودی الماس حفظ شدند.\n\n"
                    "هر زمان خواستید، با همین اکانت یا یک اکانت تلگرام دیگر دوباره وصل شوید 👇",
                    reply_markup=types.InlineKeyboardMarkup(row_width=1).add(
                        types.InlineKeyboardButton(f"{pe('robot', '🤖')} وصل کردن دوباره سلف", callback_data="reg_start", style="success")
                    ),
                )
            ...
            print(f"{pe('cross_red', '❌')} خطا در callback_remove_self_yes: {e}")
            try:
                _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} خطا: {str(e)[:80]}", show_alert=True)
            except Exception:
                pass
    ...
    def cmd_start(message):
        try:
            ...
            if not account:
                ...
                markup.add(
                    types.InlineKeyboardButton(f"{pe('robot', '🤖')} ساخت اکانت با ربات", callback_data="reg_start", style="success")
                )
                if site_url:
                    # 🔵 دکمه ساخت با وب‌سایت با رنگ primary (آبی)
                    markup.add(types.InlineKeyboardButton(f"{pe('link_chain', '🌐')} ساخت اکانت با وب سایت", url=site_url + "/register", style="primary"))
                markup.add(types.InlineKeyboardButton(f"{pe('book', '📖')} راهنما", callback_data="guide_menu", style="primary"))
                _bot.reply_to(
                    message,
                    f"{pe('person_star', '👋')} <b>سلام!</b>\n\n"
                    f"{pe('cross_red', '❌')} اکانت نداری! برای استفاده از ربات باید اکانت بسازی:\n\n"
                    f"{pe('robot', '🤖')} <b>ساخت با ربات</b> — مستقیم از همینجا، بدون نیاز به سایت\n"
                    f"{pe('link_chain', '🌐')} <b>ساخت با وب سایت</b> — از طریق پنل وب",
                    reply_markup=markup,
                )
                return
            ...
            if message.chat.type == 'private':
                ...
                if not is_logged_in:
                    ...
                    kb_reconnect.add(
                        types.InlineKeyboardButton(f"{pe('robot', '🤖')} وصل کردن سلف", callback_data="reg_start", style="success")
                    )
                    _bot.reply_to(
                        message,
                        f"{pe('person_star', '👋')} سلام <b>{account['username']}</b>!\n\n"
                        f"{pe('warning', '⚠️')} <b>سلف شما به اکانت وصل نیست.</b>\n"
            ...
            if sub:
                ...
                sub_status = (
                    f"{pe('checkmark_green', '✅')} فعال — پلن {plan_fa}\n"
                    f"   {pe('calendar', '📅')} انقضا: {_fmt_tehran(sub_exp)}\n"
                    f"   {pe('timer_1m', '⏳')} باقی‌مانده: {_remaining_str(sub_exp)}"
                ) if is_active else f"{pe('cross_red', '❌')} اشتراک ندارید"
            else:
                sub_status = f"{pe('cross_red', '❌')} اشتراک ندارید"
            ...
            default_welcome = (
                f"{pe('person_star', '👋')} سلام {name}!\n\n"
                f"{pe('timer_1h', '🕐')} وقت تهران: {time}\n\n"
                f"{pe('diamond', '💎')} موجودی الماس: {balance}\n"
                f"{pe('lines', '📊')} کل دریافتی: {total_earned}\n\n"
                f"{pe('briefcase', '📦')} اشتراک سلف:\n{sub_status}"
            )
            ...
            if message.chat.type == 'private':
                _bot.send_message(message.chat.id, f"{pe('edit_box', '📋')} منوی اصلی:", reply_markup=_main_inline_keyboard(account))

            if message.chat.type == 'private':
                sponsors = getattr(config, 'SPONSORS', [])
                if sponsors:
                    sponsors_text = f"{pe('person_check', '🤝')} <b>اسپانسرهای رسمی پروژه:</b>\n"
                    for sp in sponsors:
                        sponsors_text += f"{pe('arrow_blue', '🔸')} @{sp['username']}\n"
            ...
            print(f"{pe('cross_red', '❌')} خطا در cmd_start: {e}")
            ...
            if db.process_referral(referrer_id, tg_id):
                ...
                if referrer_tg and _bot:
                    _bot.send_message(referrer_tg, 
                        f"{pe('gift', '🎉')} یک نفر با لینک شما عضو شد!\n"
                        f"<b>+{config.REFERRAL_TOKENS} الماس</b> دریافت کردید {pe('diamond', '💎')}")
        except Exception as e:
            print(f"{pe('cross_red', '❌')} خطا در رفرال: {e}")
    ...
    def callback_check_join(call):
        try:
            ...
            if is_member:
                _bot.answer_callback_query(call.id, f"عضویت تأیید شد! {pe('checkmark_green', '✅')}")
                try: 
            ...
            print(f"{pe('cross_red', '❌')} خطا در callback_check_join: {e}")

    # ══════════════════════════════════════════════════════════════════════════
    # دکمه‌های منوی اصلی
    # ══════════════════════════════════════════════════════════════════════════
    @_bot.message_handler(func=lambda m: m.text == f"{pe('diamond', '💎')} موجودی", chat_types=['private'])
    def cmd_balance(message):
        try:
            ...
            account = _get_account_cached(message.from_user.id)
            if not account:
                return _bot.reply_to(message, f"{pe('warning', '⚠️')} ابتدا در پنل وب ثبت‌نام کنید.", reply_markup=_main_inline_keyboard())
            stats = db.get_token_stats(account["id"])
            ...
            balance_text = (
                f"{pe('diamond', '💎')} <b>موجودی الماس</b>\n\n"
                f"{pe('gift', '💰')} فعلی: <b>{stats['balance']}</b>\n"
                f"{pe('lines', '📊')} کل: <b>{stats['total_earned']}</b>\n"
                f"{pe('person_check', '👥')} رفرال: <b>{ref_count}</b> نفر\n"
                f"{pe('crown', '💵')} قیمت هر الماس: <b>{token_price} تومان</b>"
            )
            send_emoji_message(_bot, message.chat.id, balance_text,
                                reply_to_message_id=message.message_id,
                                reply_markup=_main_inline_keyboard(account))
        except Exception as ex:
            print(f"{pe('cross_red', '❌')} خطا در cmd_balance: {ex}")
    ...
    def callback_menu_balance(call):
        try:
            ...
            account = _get_account_cached(call.from_user.id)
            if not account:
                return _bot.answer_callback_query(call.id, f"{pe('warning', '⚠️')} ابتدا در پنل وب ثبت‌نام کنید.", show_alert=True)
            stats = db.get_token_stats(account["id"])
            ref_count = db.get_referral_count(account["id"])
            token_price = getattr(config, 'TOKEN_PRICE_TOMAN', 200)
            _bot.answer_callback_query(call.id)
            ...
            print(f"{pe('cross_red', '❌')} خطا در callback_menu_balance: {ex}")

    @_bot.message_handler(func=lambda m: m.text == f"{pe('gift', '🎁')} هدیه روزانه", chat_types=['private'])
    ...
    def _do_daily(tg_id, chat_id, reply_to=None):
        try:
            account = _get_account_cached(tg_id)
            if not account:
                return _bot.send_message(chat_id, f"{pe('warning', '⚠️')} ابتدا در پنل وب ثبت‌نام کنید.", reply_markup=_main_inline_keyboard())
            success, msg = db.claim_daily_token(account["id"])
            cache.invalidate(f"account_{tg_id}")
            if success:
                stats = db.get_token_stats(account["id"])
                text = f"{msg}\n\n{pe('diamond', '💎')} موجودی جدید: <b>{stats['balance']}</b>"
            else:
                text = msg
            ...
            print(f"{pe('cross_red', '❌')} خطا در _do_daily: {e}")

    @_bot.message_handler(func=lambda m: m.text == f"{pe('link_chain', '🔗')} رفرال", chat_types=['private'])
    ...
    def _do_referral(tg_id, chat_id, reply_to=None):
        try:
            ...
            _bot.send_message(chat_id,
                f"{pe('link_chain', '🔗')} <b>لینک رفرال شما:</b>\n<code>{link}</code>\n\n"
                f"{pe('group_add', '👥')} تعداد: <b>{ref_count}</b>\n"
                f"{pe('gift', '🎁')} پاداش: <b>{config.REFERRAL_TOKENS} الماس</b> (معادل {referral_value} تومان)",
                **kwargs)
        except Exception as e:
            print(f"{pe('cross_red', '❌')} خطا در _do_referral: {e}")
    ...
    def _purchase_main_keyboard():
        ...
        markup.add(
            types.InlineKeyboardButton(f"{pe('diamond', '💎')} خرید اشتراک با الماس", callback_data="pur_sub_diamond", style="success"),
        )
        ...
        markup.add(
            types.InlineKeyboardButton(f"{pe('briefcase', '🛍')} خرید الماس", callback_data="pur_buy_diamond", style="success"),
        )
        return markup
    ...
    def _plans_keyboard(prefix: str):
        ...
        markup.add(types.InlineKeyboardButton(f"{pe('arrow_blue', '🔙')} بازگشت", callback_data="pur_back", style="danger"))
        return markup

    @_bot.message_handler(func=lambda m: m.text and m.text.strip() in ("خرید", f"{pe('inbox', '🛒')} خرید الماس", f"{pe('inbox', '🛒')} خرید"), chat_types=['private'])
    def cmd_buy(message):
    ...
    def _do_buy(tg_id, chat_id, reply_to=None):
        try:
            account = _get_account_cached(tg_id)
            if not account:
                return _bot.send_message(chat_id, f"{pe('cross_red', '❌')} ابتدا در پنل وب ثبت‌نام کنید.", reply_markup=_main_inline_keyboard())
            balance = db.get_token_balance(account["id"])
            kwargs = {"reply_markup": _purchase_main_keyboard()}
            if reply_to:
                kwargs["reply_to_message_id"] = reply_to
            _bot.send_message(chat_id,
                f"{pe('inbox', '🛒')} <b>منوی خرید</b>\n\n"
                f"{pe('diamond', '💎')} موجودی فعلی شما: <b>{balance} الماس</b>\n\n"
                f"یکی از گزینه‌های زیر را انتخاب کنید:",
                **kwargs)
        except Exception as e:
            print(f"{pe('cross_red', '❌')} خطا در _do_buy: {e}")
    ...
    def callback_purchase(call):
        try:
            ...
            # ── اشتراک با الماس ─────────────────────────────────────────────
                ...
                balance = db.get_token_balance(account["id"])
                text = (
                    f"{pe('diamond', '💎')} <b>خرید اشتراک با الماس</b>\n\n"
                ...
                if not plan:
                    return _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} پلن نامعتبر", show_alert=True)
                balance = db.get_token_balance(account["id"])
                cost = plan["diamonds"]
                if balance < cost:
                    need = cost - balance
                    markup = types.InlineKeyboardMarkup()
                    # 🟢 دکمه خرید الماس با رنگ success (سبز)
                    markup.add(types.InlineKeyboardButton(f"{pe('briefcase', '🛍')} خرید الماس", callback_data="pur_buy_diamond", style="success"))
                    # 🔴 دکمه بازگشت با رنگ danger (قرمز)
                    markup.add(types.InlineKeyboardButton(f"{pe('arrow_blue', '🔙')} بازگشت", callback_data="pur_sub_diamond", style="danger"))
                    return _bot.edit_message_text(
                        f"{pe('cross_red', '❌')} <b>موجودی کافی نیست!</b>\n\n"
                        f"{pe('diamond', '💎')} موجودی: {balance} الماس\n"
                        f"{pe('diamond', '💎')} نیاز: {cost} الماس\n"
                        f"{pe('diamond', '💎')} کمبود: {need} الماس\n\n"
                        f"{pe('bulb', '💡')} برای کسب الماس:\n"
                        f"• دریافت هدیه روزانه {pe('gift', '🎁')}\n"
                        f"• دعوت دوستان {pe('link_chain', '🔗')}\n"
                        f"• خرید الماس {pe('briefcase', '🛍')}",
                        chat_id=call.message.chat.id, message_id=call.message.message_id,
                        reply_markup=markup
                    )
                ...
                _bot.edit_message_text(
                    f"{pe('checkmark_green', '✅')} <b>اشتراک {plan['fa']} فعال شد!</b>\n\n"
                    f"{pe('diamond', '💎')} {cost} الماس کسر شد\n"
                    f"{pe('calendar', '📅')} انقضا: <b>{exp_str}</b>",
                    chat_id=call.message.chat.id, message_id=call.message.message_id
                )
                _bot.answer_callback_query(call.id, f"{pe('checkmark_green', '✅')} اشتراک {plan['fa']} فعال شد!", show_alert=True)
            ...
            # ── اشتراک با کارت ──────────────────────────────────────────────
                ...
                markup = types.InlineKeyboardMarkup()
                # 🔴 دکمه بازگشت با رنگ danger (قرمز)
                markup.add(types.InlineKeyboardButton(f"{pe('arrow_blue', '🔙')} بازگشت", callback_data="pur_sub_card", style="danger"))
                _bot.edit_message_text(
                    f"💳 <b>پرداخت اشتراک {plan['fa']}</b>\n\n"
                    f"{pe('diamond', '💰')} مبلغ: <b>{plan['toman']:,} تومان</b>\n"
                    f"💳 شماره کارت: <code>{card}</code>\n"
                    f"{pe('person_star', '👤')} به نام: <b>غفاری</b>\n\n"
            ...
            # ── خرید الماس ──────────────────────────────────────────────────
                ...
                markup.add(types.InlineKeyboardButton(f"{pe('arrow_blue', '🔙')} بازگشت", callback_data="pur_back", style="danger"))
                _bot.edit_message_text(
                    f"{pe('briefcase', '🛍')} <b>خرید الماس</b>\n\n"
                    f"{pe('diamond', '💎')} نرخ: هر ۱۰۰ الماس = <b>{100 * DIAMOND_RATE:,} تومان</b>\n"
                    f"{pe('location', '📌')} حداقل خرید: <b>{DIAMOND_MIN_BUY} الماس</b>\n\n"
            ...
            # ── تأیید/رد پرداخت توسط ادمین ─────────────────────────────────
                ...
                if tg_id != OWNER_TG_ID:
                    return _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} فقط مالک دسترسی دارد", show_alert=True)
                action = "approve" if data.startswith("pur_approve_") else "reject"
                payment_id = int(data.split("_")[2])
                payment = db.get_payment(payment_id)
                if not payment:
                    return _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} پرداخت یافت نشد", show_alert=True)
                if payment["status"] != "pending":
                    return _bot.answer_callback_query(call.id, f"{pe('warning', '⚠️')} این پرداخت قبلاً پردازش شده", show_alert=True)
                ...
                if action == "approve":
                    ...
                    if payment["type"] == "subscription":
                        ...
                        try:
                            _bot.send_message(
                                payment["tg_id"],
                                f"{pe('checkmark_green', '✅')} <b>پرداخت تأیید شد!</b>\n\n"
                                f"{pe('gift', '🎉')} اشتراک {plan.get('fa','')  } شما فعال شد\n"
                                f"{pe('calendar', '📅')} انقضا: <b>{exp_str}</b>"
                            )
                        ...
                        try:
                            _bot.send_message(
                                payment["tg_id"],
                                f"{pe('checkmark_green', '✅')} <b>پرداخت تأیید شد!</b>\n\n"
                                f"{pe('diamond', '💎')} <b>{amount} الماس</b> به حسابتان اضافه شد!"
                            )
                        except Exception: pass
                    ...
                    _bot.edit_message_reply_markup(
                        ...
                        message_id=call.message.message_id,
                        reply_markup=types.InlineKeyboardMarkup().add(
                            types.InlineKeyboardButton(f"{pe('checkmark_green', '✅')} تأیید شد", callback_data="noop", style="success")
                        )
                    )
                    _bot.answer_callback_query(call.id, f"{pe('checkmark_green', '✅')} پرداخت تأیید شد!", show_alert=True)
                    ...
                    _bot.edit_message_reply_markup(
                        ...
                        message_id=call.message.message_id,
                        reply_markup=types.InlineKeyboardMarkup().add(
                            types.InlineKeyboardButton(f"{pe('cross_red', '❌')} رد شد", callback_data="noop", style="danger")
                        )
                    )
                    _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} پرداخت رد شد", show_alert=True)

            elif data == "noop":
                _bot.answer_callback_query(call.id)

        except Exception as e:
            print(f"{pe('cross_red', '❌')} خطا در callback_purchase: {e}")
            _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} خطا: {str(e)[:80]}", show_alert=True)
        ...
        try:
            ...
            if step == "waiting_diamond_amount":
                try:
                    ...
                    return _bot.reply_to(message, f"{pe('cross_red', '❌')} لطفاً یک عدد معتبر وارد کنید.")
                if amount < DIAMOND_MIN_BUY:
                    return _bot.reply_to(message, f"{pe('cross_red', '❌')} حداقل {DIAMOND_MIN_BUY} الماس باید خرید.")
                ...
                markup = types.InlineKeyboardMarkup()
                # 🔴 دکمه لغو با رنگ danger (قرمز)
                markup.add(types.InlineKeyboardButton(f"{pe('cross_red', '❌')} لغو", callback_data="pur_back", style="danger"))
                _bot.reply_to(message,
                    f"{pe('briefcase', '🛍')} <b>خرید {amount} الماس</b>\n\n"
                    f"{pe('diamond', '💰')} مبلغ: <b>{toman:,} تومان</b>\n"
                    f"💳 شماره کارت: <code>{card}</code>\n"
                    f"{pe('person_star', '👤')} به نام: <b>غفاری</b>\n\n"
                    f"بعد از واریز، تصویر رسید را ارسال کنید 👇",
                    reply_markup=markup
            ...
            # ── کاربر رسید فرستاد ────────────────────────────────────────
                ...
                if message.photo:
                    ...
                    return _bot.reply_to(message, f"{pe('cross_red', '❌')} لطفاً تصویر رسید را ارسال کنید.")
                ...
                admin_text = (
                    f"🧾 <b>رسید جدید</b>\n\n"
                    f"{pe('person_star', '👤')} کاربر: {user_display}\n"
                    f"🆔 تلگرام: <code>{tg_id}</code>\n"
                    f"{pe('briefcase', '📦')} نوع: {desc}\n"
                    f"🔢 شناسه پرداخت: <code>{payment_id}</code>"
                )
                admin_markup = types.InlineKeyboardMarkup(row_width=2)
                # 🟢 دکمه تأیید با رنگ success (سبز)
                admin_markup.add(
                    types.InlineKeyboardButton(f"{pe('checkmark_green', '✅')} تأیید", callback_data=f"pur_approve_{payment_id}", style="success"),
                )
                # 🔴 دکمه رد با رنگ danger (قرمز)
                admin_markup.add(
                    types.InlineKeyboardButton(f"{pe('cross_red', '❌')} رد", callback_data=f"pur_reject_{payment_id}", style="danger")
                )
                try:
                    ...
                    print(f"{pe('cross_red', '❌')} ارسال رسید به ادمین: {e}")

                _purchase_states.pop(tg_id, None)
                _bot.reply_to(message,
                    f"{pe('checkmark_green', '✅')} <b>رسید دریافت شد!</b>\n\n"
                    f"{pe('timer_1m', '⏳')} پس از تأیید توسط ادمین، اشتراک/الماس شما فعال می‌شود.\n"
                    "معمولاً کمتر از ۳۰ دقیقه طول می‌کشد."
                )

        except Exception as e:
            print(f"{pe('cross_red', '❌')} خطا در handle_purchase_state: {e}")

    # ══════════════════════════════════════════════════════════════════════════
    # 📢 پنل مدیریت مالک
    # ══════════════════════════════════════════════════════════════════════════
    @_bot.message_handler(func=lambda m: m.text == f"{pe('megaphone', '📢')} مدیریت", chat_types=['private'])
    def cmd_admin_panel(message):
        if message.from_user.id != OWNER_TG_ID:
            return
        _bot.reply_to(message, 
            f"{pe('megaphone', '📢')} <b>پنل مدیریت مالک</b>\n\nیکی از گزینه‌های زیر را انتخاب کنید:",
    ...
    def callback_admin(call):
        ...
        if uid != OWNER_TG_ID:
            ...
            if not db.is_sub_admin(uid):
                return _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} دسترسی ندارید", show_alert=True)
            ...
            if data.startswith("admin_perm_") or data == "admin_manage_admins" or data == "admin_add_admin" or data.startswith("admin_del_admin_"):
                return _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} این بخش فقط برای مالک است", show_alert=True)
            if data not in ("admin_panel", "admin_back"):
                ...
                if perm is None or not db.sub_admin_has_permission(uid, perm):
                    return _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} شما به این بخش دسترسی ندارید", show_alert=True)
        ...
        try:
            if data == "admin_panel" or data == "admin_back":
                ...
                markup = types.InlineKeyboardMarkup(row_width=1)
                if channels:
                    text = f"{pe('megaphone', '📢')} <b>چنل‌های اجباری فعلی:</b>\n\n"
                    for ch in channels:
                        text += f"{pe('arrow_blue', '🔸')} <code>{ch}</code>\n"
                        ch_clean = ch.lstrip("@")
                        # 🔴 دکمه حذف با رنگ danger (قرمز)
                        markup.add(types.InlineKeyboardButton(f"{pe('cross_red', '❌')} حذف {ch}", callback_data=f"rmch_{ch_clean}", style="danger"))
                else:
                    text = f"{pe('edit_box', '📋')} لیست چنل‌ها خالی است.\n\n"
                ...
                # 🔴 دکمه بازگشت با رنگ danger (قرمز)
                markup.add(types.InlineKeyboardButton(f"{pe('arrow_blue', '🔙')} بازگشت", callback_data="admin_panel", style="danger"))
                _bot.edit_message_text(
                    text,
                    chat_id=call.message.chat.id,
                    message_id=call.message.message_id,
                ...
                if db.remove_forced_channel(ch):
                    ...
                    _bot.answer_callback_query(call.id, f"{pe('checkmark_green', '✅')} چنل {ch} حذف شد")
                    call.data = "admin_channels"
                    callback_admin(call)
                else:
                    _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} خطا در حذف")
                return
                ...
                markup = types.InlineKeyboardMarkup()
                # 🔴 دکمه لغو با رنگ danger (قرمز)
                markup.add(types.InlineKeyboardButton(f"{pe('cross_red', '❌')} لغو", callback_data="admin_panel", style="danger"))
                _bot.edit_message_text(
                    f"{pe('edit_box', '📝')} آیدی چنل را ارسال کنید (با @ شروع شود):\n\nمثال: <code>@mychannel</code>",
                    chat_id=call.message.chat.id,
                    message_id=call.message.message_id,
                    reply_markup=markup
                )
                ...
                lines = [f"{pe('group_add', '👥')} <b>کاربران ({total} نفر) — صفحه {page}/{total_pages}</b>\n"]
                for i, acc in enumerate(page_accounts, start_idx + 1):
                    ...
                    if tg_id_val:
                        ...
                        tg_id_line = f"{pe('phone', '📱')} <a href='{pv_link}'>{username_part}پیوی کاربر</a> (<code>{tg_id_val}</code>)"
                    else:
                        tg_id_line = f"{pe('phone', '📱')} تلگرام: ─"
                    lines.append(
                        f"┌─ <b>#{i} {acc['username']}</b>\n"
                        f"├ 🆔 پنل: <code>{acc['id']}</code>\n"
                        f"├ {tg_id_line}\n"
                        f"├ {pe('diamond', '💎')} موجودی: <b>{bal} الماس</b>\n"
                        f"└ {pe('timer_1m', '⏳')} پلن: {remaining}"
                    )
                text = "\n\n".join(lines)
                ...
                markup.add(types.InlineKeyboardButton(f"{pe('edit_box', '📋')} چالش‌های فعال", callback_data="wc_list", style="primary"))
                # 🔴 دکمه بازگشت با رنگ danger (قرمز)
                markup.add(types.InlineKeyboardButton(f"{pe('arrow_blue', '🔙')} بازگشت", callback_data="admin_panel", style="danger"))
                _bot.edit_message_text(
                    f"{pe('trophy', '🏆')} <b>مدیریت چالش‌های جام جهانی</b>\n\nیک گزینه را انتخاب کنید:",
                    chat_id=call.message.chat.id,
                    message_id=call.message.message_id,
                    reply_markup=markup
                )
                ...
                markup = types.InlineKeyboardMarkup()
                # 🔴 دکمه لغو با رنگ danger (قرمز)
                markup.add(types.InlineKeyboardButton(f"{pe('cross_red', '❌')} لغو", callback_data="admin_wc", style="danger"))
                _bot.edit_message_text(
                    f"{pe('trophy', '🏆')} <b>ایجاد چالش جدید</b>\n\n"
                    f"{pe('edit_box', '📝')} مرحله ۱ از ۴:\nنام <b>تیم اول</b> را ارسال کنید:\n\nمثال: <code>ایران</code>",
                    chat_id=call.message.chat.id,
                    message_id=call.message.message_id,
                    reply_markup=markup
                )
                ...
                if not challenges:
                    text = f"{pe('edit_box', '📋')} هیچ چالش فعالی وجود ندارد."
                    markup = types.InlineKeyboardMarkup()
                    # 🔴 دکمه بازگشت با رنگ danger (قرمز)
                    markup.add(types.InlineKeyboardButton(f"{pe('arrow_blue', '🔙')} بازگشت", callback_data="admin_wc", style="danger"))
                else:
                    text = f"{pe('trophy', '🏆')} <b>چالش‌های فعال:</b>\n\n"
                    markup = types.InlineKeyboardMarkup(row_width=2)
                    for c in challenges:
                        text += f"<b>ID {c['id']}:</b> {c['team1']} vs {c['team2']}\n"
                        text += f"{pe('timer_1h', '⏰')} {c['match_time']} | {pe('diamond', '💎')} {c['bet_amount']}\n\n"
                        # 🟢 دکمه‌های تعیین برنده با رنگ success (سبز)
                        markup.add(
                            types.InlineKeyboardButton(f"{pe('checkmark_green', '✅')} {c['team1']}", callback_data=f"wcwin_{c['id']}_{c['team1']}", style="success"),
                            types.InlineKeyboardButton(f"{pe('checkmark_green', '✅')} {c['team2']}", callback_data=f"wcwin_{c['id']}_{c['team2']}", style="success")
                        )
                    # 🔴 دکمه بازگشت با رنگ danger (قرمز)
                    markup.add(types.InlineKeyboardButton(f"{pe('arrow_blue', '🔙')} بازگشت", callback_data="admin_wc", style="danger"))
                _bot.edit_message_text(
                ...
                if success:
                    ...
                    _bot.answer_callback_query(call.id, f"{pe('checkmark_green', '✅')} برنده: {winner_team}\n{pe('trophy', '🏆')} {won_count} برنده | {pe('cross_red', '❌')} {lost_count} بازنده", show_alert=True)
                    for r in results:
                        if r["result"] == "won":
                            try:
                                _bot.send_message(r["user_tg_id"], f"{pe('gift', '🎉')} تبریک! شرط شما درست بود.\n{pe('diamond', '💎')} <b>{r['amount']} الماس</b> دریافت کردید.")
                            except: 
                                pass
                else:
                    _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} خطا: {results}", show_alert=True)
                return
            
            elif data == "admin_today_games":
                _bot.answer_callback_query(call.id, f"{pe('timer_1m', '⏳')} در حال دریافت بازی‌های امروز...")
                try:
                    today_matches = _wc_get_today_matches()
                except Exception as e:
                    today_matches = None
                    print(f"{pe('cross_red', '❌')} خطا در دریافت بازی‌های امروز: {e}")

                if today_matches is None:
                    text = f"{pe('cross_red', '❌')} خطا در ارتباط با Football API.\nلاگ سرور را بررسی کنید."
                    markup = types.InlineKeyboardMarkup()
                    # 🔴 دکمه بازگشت با رنگ danger (قرمز)
                    markup.add(types.InlineKeyboardButton(f"{pe('arrow_blue', '🔙')} بازگشت", callback_data="admin_panel", style="danger"))
                    try:
                ...
                if not getattr(config, "FOOTBALL_API_KEY", ""):
                    text = f"{pe('warning', '⚠️')} FOOTBALL_API_KEY تنظیم نشده است."
                    markup = types.InlineKeyboardMarkup()
                    # 🔴 دکمه بازگشت با رنگ danger (قرمز)
                    markup.add(types.InlineKeyboardButton(f"{pe('arrow_blue', '🔙')} بازگشت", callback_data="admin_panel", style="danger"))
                    _bot.edit_message_text(text, chat_id=call.message.chat.id,
                ...
                status_fa = {
                    "SCHEDULED": f"{pe('timer_1m', '⏳')}", "TIMED": f"{pe('timer_1m', '⏳')}",
                    "LIVE": f"{pe('exclamation_red', '🔴')}", "IN_PLAY": f"{pe('exclamation_red', '🔴')}", "PAUSED": "⏸️",
                    "FINISHED": f"{pe('checkmark_green', '✅')}", "POSTPONED": f"{pe('location', '📌')}",
                    "SUSPENDED": "⛔️", "CANCELLED": f"{pe('cross_red', '❌')}",
                }
                lines = [f"{pe('calendar', '📅')} <b>بازی‌های امروز — جام جهانی</b>\n"]
                markup = types.InlineKeyboardMarkup(row_width=1)
                ...
                for m in today_matches:
                    ...
                    lines.append(f"{st} <b>{home}</b> vs <b>{away}</b> — {pe('timer_1h', '⏰')}{time_str}")
                ...
                _bot.answer_callback_query(call.id, f"{pe('timer_1m', '⏳')} در حال ارسال چالش...")
                try:
                    today_matches = _wc_get_today_matches()
                    target = next((m for m in today_matches if str(m.get("id")) == match_id), None)
                    if not target:
                        return _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} بازی یافت نشد", show_alert=True)
                    ...
                    if not home.strip() or not away.strip():
                        return _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} نام تیم‌ها هنوز مشخص نیست", show_alert=True)
                    ...
                    if db.wc_challenge_exists(match_id):
                        ...
                        if row:
                            ...
                            _bot.answer_callback_query(call.id, f"{pe('checkmark_green', '✅')} چالش مجدداً ارسال شد!", show_alert=True)
                            return

                    challenge_id = db.create_wc_challenge(match_id, home, away, dt)
                    if challenge_id:
                        _wc_send_challenge_to_channel(challenge_id, home, away, match_time_str)
                        _bot.answer_callback_query(call.id, f"{pe('checkmark_green', '✅')} چالش {home} vs {away} ارسال شد!", show_alert=True)
                    else:
                        _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} خطا در ساخت چالش", show_alert=True)
                except Exception as e:
                    print(f"{pe('cross_red', '❌')} wc_sendnow: {e}")
                    _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} خطا: {str(e)[:80]}", show_alert=True)
                return
                ...
                markup.add(types.InlineKeyboardButton(f"{pe('cross_red', '❌')} لغو", callback_data="admin_panel", style="danger"))
                _bot.edit_message_text(
                    f"{pe('diamond', '💎')} <b>انتقال الماس (از طرف سیستم)</b>\n\n"
                    f"{pe('edit_box', '📝')} یوزرنیم کاربر مقصد را ارسال کنید:\n\nمثال: <code>ali</code>",
                    chat_id=call.message.chat.id,
                    message_id=call.message.message_id,
                    reply_markup=markup
                )
                ...
                markup.add(types.InlineKeyboardButton(f"{pe('cross_red', '❌')} لغو", callback_data="admin_panel", style="danger"))
                _bot.edit_message_text(
                    f"{pe('diamond', '💰')} <b>دادن الماس به کاربر</b>\n\n"
                    f"{pe('edit_box', '📝')} یوزرنیم کاربر را ارسال کنید:\n\nمثال: <code>ali</code>",
                    chat_id=call.message.chat.id,
                    message_id=call.message.message_id,
                    reply_markup=markup
                )
                ...
                if not payments:
                    _bot.answer_callback_query(call.id, f"{pe('checkmark_green', '✅')} هیچ پرداخت معلقی وجود ندارد", show_alert=True)
                    return
                ...
                markup.add(types.InlineKeyboardButton(f"{pe('cross_red', '❌')} لغو", callback_data="admin_panel", style="danger"))
                _bot.edit_message_text(
                    f"{pe('megaphone', '📣')} <b>ارسال پیام عمومی</b>\n\n"
                ...
                if not participants:
                    ...
                    for i, p in enumerate(participants[:50], 1):
                        uname = f"@{p['username']}"
                        lines.append(f"{i}. <b>{uname}</b> — {pe('trophy', '🎯')}{p['bet_count']} شرط | {pe('diamond', '💎')}{p['total_bet']} الماس")
                    text = "\n".join(lines)
                markup = types.InlineKeyboardMarkup()
                # 🔴 دکمه بازگشت با رنگ danger (قرمز)
                markup.add(types.InlineKeyboardButton(f"{pe('arrow_blue', '🔙')} بازگشت", callback_data="admin_panel", style="danger"))
                ...
                markup = types.InlineKeyboardMarkup(row_width=2)
                markup.add(
                    types.InlineKeyboardButton(f"{pe('diamond', '💎')} الماس", callback_data="admin_gift_diamond", style="primary"),
                    types.InlineKeyboardButton(f"{pe('edit_box', '📋')} پنل", callback_data="admin_gift_panel", style="success")
                )
                markup.add(types.InlineKeyboardButton(f"{pe('cross_red', '❌')} لغو", callback_data="admin_panel", style="danger"))
                _bot.edit_message_text(
                    f"{pe('gift', '🎁')} <b>هدیه به کاربر</b>\n\n"
                ...
                markup = types.InlineKeyboardMarkup(row_width=1)
                markup.add(
                    types.InlineKeyboardButton(f"{pe('calendar', '📅')} پنل یک ماهه (30 روز)", callback_data="admin_gift_plan_30", style="primary"),
                    types.InlineKeyboardButton(f"{pe('calendar', '📅')} پنل یک هفته‌ای (7 روز)", callback_data="admin_gift_plan_7", style="primary"),
                    types.InlineKeyboardButton(f"{pe('calendar', '📅')} پنل یک روزه (1 روز)", callback_data="admin_gift_plan_1", style="primary")
                )
                markup.add(types.InlineKeyboardButton(f"{pe('cross_red', '❌')} لغو", callback_data="admin_panel", style="danger"))
                _bot.edit_message_text(
                    f"{pe('edit_box', '📋')} <b>هدیه پنل</b>\n\n"
                ...
                markup.add(types.InlineKeyboardButton(f"{pe('cross_red', '❌')} لغو", callback_data="admin_panel", style="danger"))
                _bot.edit_message_text(
                    f"{pe('edit_box', '📋')} <b>پنل {plan_label}</b>\n\n"
                ...
                if not gift_info or gift_info.get("key") != gift_key:
                    _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} اطلاعات هدیه منقضی شده. دوباره تلاش کنید.", show_alert=True)
                    return
                ...
                if gift_type == "diamond":
                    ...
                    gift_desc = f"{pe('diamond', '💎')} {amount} الماس"
                    try:
                        _bot.send_message(
                            tg_id,
                            f"{pe('gift', '🎁')} <b>تبریک! شما از طرف مالک هدیه گرفتید!</b>\n\n"
                            f"{pe('gift', '🎊')} مشخصات هدیه:\n"
                            f"╔══════════════════╗\n"
                            f"  {pe('diamond', '💎')} <b>الماس هدیه:</b> {amount} الماس\n"
                            f"  {pe('diamond', '💰')} <b>موجودی جدید:</b> {new_balance} الماس\n"
                            f"╚══════════════════╝"
                        )
                    except Exception:
                        pass
                    admin_msg = (
                        f"{pe('checkmark_green', '✅')} <b>هدیه با موفقیت ارسال شد!</b>\n\n"
                        f"{pe('person_star', '👤')} کاربر: <b>{account['username']}</b>\n"
                        f"{pe('diamond', '💎')} هدیه: <b>{amount} الماس</b>\n"
                        f"{pe('diamond', '💰')} موجودی جدید: <b>{new_balance}</b>"
                    )
                else:
                    days = gift_info["days"]
                    ...
                    gift_desc = f"{pe('edit_box', '📋')} پنل {plan_label}"
                    try:
                        _bot.send_message(
                            tg_id,
                            f"{pe('gift', '🎁')} <b>تبریک! شما از طرف مالک هدیه گرفتید!</b>\n\n"
                            f"{pe('gift', '🎊')} مشخصات هدیه:\n"
                            f"╔══════════════════╗\n"
                            f"  {pe('edit_box', '📋')} <b>پنل هدیه:</b> {plan_label} ({days} روز)\n"
                            f"  {pe('calendar', '📅')} <b>تاریخ انقضا:</b> {end_date}\n"
                            f"╚══════════════════╝"
                        )
                    except Exception:
                        pass
                    admin_msg = (
                        f"{pe('checkmark_green', '✅')} <b>هدیه با موفقیت ارسال شد!</b>\n\n"
                        f"{pe('person_star', '👤')} کاربر: <b>{account['username']}</b>\n"
                        f"{pe('edit_box', '📋')} هدیه: <b>پنل {plan_label}</b>\n"
                        f"{pe('calendar', '📅')} انقضا: <b>{end_date}</b>"
                    )

                _owner_states.pop(call.from_user.id, None)
                markup = types.InlineKeyboardMarkup()
                markup.add(types.InlineKeyboardButton(f"{pe('arrow_blue', '🔙')} پنل مدیریت", callback_data="admin_panel", style="primary"))
                _bot.edit_message_text(
                    admin_msg,
                    chat_id=call.message.chat.id,
                    message_id=call.message.message_id,
                ...
                _bot.answer_callback_query(call.id, f"{pe('checkmark_green', '✅')} هدیه ارسال شد!", show_alert=False)
                return
                ...
                _bot.edit_message_text(
                    f"{pe('cross_red', '❌')} <b>عملیات هدیه لغو شد.</b>",
                    chat_id=call.message.chat.id,
                    message_id=call.message.message_id,
                    reply_markup=markup
                )
                ...
                markup = types.InlineKeyboardMarkup()
                markup.add(types.InlineKeyboardButton(f"{pe('cross_red', '❌')} لغو", callback_data="admin_guide_manage", style="danger"))
                _bot.edit_message_text(
                ...
                markup.add(types.InlineKeyboardButton(f"{pe('cross_red', '❌')} لغو", callback_data="admin_guide_manage", style="danger"))
                _bot.edit_message_text(
                    f"{pe('edit_box', '📝')} <b>ارسال آموزش متنی</b>\n\n"
                ...
                markup = types.InlineKeyboardMarkup(row_width=1)
                if guides:
                    txt = f"{pe('book', '📚')} <b>راهنماهای ثبت‌شده ({len(guides)} آموزش):</b>\n\n"
                    for i, g in enumerate(guides):
                        txt += f"{'🎥' if g['type'] == 'video' else '🖼' if g['type'] == 'photo' else '📝'} {g['name']}\n"
                        markup.add(types.InlineKeyboardButton(
                            f"{pe('cross_red', '❌')} حذف «{g['name']}»", callback_data=f"admin_guide_del_{i}", style="danger"))
                else:
                    txt = f"{pe('book', '📚')} <b>مدیریت راهنما</b>\n\nهیچ آموزشی ثبت نشده."
                ...
                _bot.edit_message_text(txt, chat_id=call.message.chat.id,
                    message_id=call.message.message_id, reply_markup=markup)
                _bot.answer_callback_query(call.id, f"{pe('checkmark_green', '✅')} آموزش حذف شد")
                return
                ...
                markup = types.InlineKeyboardMarkup(row_width=1)
                markup.add(
                    types.InlineKeyboardButton(f"{pe('pencil', '✏️')} تغییر متن خوش‌آمد", callback_data="admin_welcome_edit_text", style="primary"),
                ...
                info = (
                    f"{pe('pencil', '✏️')} <b>تنظیمات پیام خوش‌آمد</b>\n\n"
                    f"{'🖼 عکس: تنظیم شده ✅' if cur_photo else '🖼 عکس: تنظیم نشده ❌'}\n\n"
                    f"{pe('edit_box', '📝')} <b>متن فعلی:</b>\n<code>{preview or '(پیش‌فرض)'}</code>\n\n"
                    "━━━━━━━━━━━━━━━━\n"
                    f"{pe('location', '📌')} <b>متغیرهای قابل استفاده:</b>\n"
                ...
                markup = types.InlineKeyboardMarkup()
                markup.add(types.InlineKeyboardButton(f"{pe('cross_red', '❌')} لغو", callback_data="admin_welcome_settings", style="danger"))
                _bot.edit_message_text(
                    f"{pe('pencil', '✏️')} <b>تغییر متن خوش‌آمد</b>\n\n"
                ...
                markup = types.InlineKeyboardMarkup()
                markup.add(types.InlineKeyboardButton(f"{pe('arrow_blue', '🔙')} بازگشت", callback_data="admin_welcome_settings", style="danger"))
                _bot.edit_message_text(f"{pe('checkmark_green', '✅')} عکس خوش‌آمد حذف شد.",
                    chat_id=call.message.chat.id, message_id=call.message.message_id, reply_markup=markup)
                _bot.answer_callback_query(call.id, f"{pe('checkmark_green', '✅')} حذف شد")
                return
                ...
                markup = types.InlineKeyboardMarkup()
                markup.add(types.InlineKeyboardButton(f"{pe('arrow_blue', '🔙')} بازگشت", callback_data="admin_welcome_settings", style="danger"))
                _bot.edit_message_text(f"{pe('checkmark_green', '✅')} متن و عکس خوش‌آمد به پیش‌فرض بازگشت.",
                    chat_id=call.message.chat.id, message_id=call.message.message_id, reply_markup=markup)
                _bot.answer_callback_query(call.id)
                return
                ...
                try:
                    preview_text = template.format(
                        ...
                        balance="999",
                        total_earned="9999",
                        sub_status=f"{pe('checkmark_green', '✅')} فعال — پیش‌نمایش",
                    )
                except Exception as fmt_err:
                    preview_text = f"{pe('cross_red', '❌')} خطا در قالب‌بندی: {fmt_err}"
                welcome_photo = db.get_global_setting("welcome_photo_id", "")
                markup = types.InlineKeyboardMarkup()
                markup.add(types.InlineKeyboardButton(f"{pe('arrow_blue', '🔙')} بازگشت", callback_data="admin_welcome_settings", style="danger"))
                if welcome_photo:
                ...
                markup = types.InlineKeyboardMarkup(row_width=1)
                if admins:
                    text_lines = [f"{pe('shield', '👮')} <b>ادمین‌های فرعی ({len(admins)} نفر):</b>\n"]
                    for a in admins:
                        ...
                        markup.add(types.InlineKeyboardButton(
                            f"{pe('key', '🔑')} دسترسی‌های {name}", callback_data=f"admin_perm_edit_{tg_id_a}", style="primary"
                        ))
                        markup.add(types.InlineKeyboardButton(
                            f"{pe('cross_red', '❌')} حذف {name}", callback_data=f"admin_del_admin_{tg_id_a}", style="danger"
                        ))
                    admin_text = "\n".join(text_lines)
                else:
                    admin_text = f"{pe('shield', '👮')} <b>ادمین‌های فرعی</b>\n\nهنوز هیچ ادمینی اضافه نشده."
                ...
                markup = types.InlineKeyboardMarkup()
                markup.add(types.InlineKeyboardButton(f"{pe('cross_red', '❌')} لغو", callback_data="admin_manage_admins", style="danger"))
                _bot.edit_message_text(
                    f"{pe('shield', '👮')} <b>افزودن ادمین فرعی</b>\n\n"
                ...
                if admins:
                    ...
                    for a in admins:
                        ...
                        markup.add(types.InlineKeyboardButton(
                            f"{pe('cross_red', '❌')} حذف {name}", callback_data=f"admin_del_admin_{tg_id}", style="danger"
                        ))
                    admin_text = "\n".join(text_lines)
                else:
                    admin_text = f"{pe('shield', '👮')} <b>ادمین‌های فرعی</b>\n\nهنوز هیچ ادمینی اضافه نشده."
                ...
                markup.add(types.InlineKeyboardButton(f"{pe('arrow_blue', '🔙')} بازگشت", callback_data="admin_panel", style="danger"))
                _bot.edit_message_text(
                    f"{pe('checkmark_green', '✅')} ادمین حذف شد.\n\n" + admin_text,
                    chat_id=call.message.chat.id,
                    message_id=call.message.message_id,
                    reply_markup=markup
                )
                _bot.answer_callback_query(call.id, f"{pe('checkmark_green', '✅')} ادمین حذف شد")
                return
                ...
                if not admin_info:
                    _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} ادمین یافت نشد", show_alert=True)
                    return
                ...
                for perm_key, perm_label in db.ADMIN_PERMISSIONS:
                    ...
                    icon = f"{pe('checkmark_green', '✅')}" if has_perm else "⬜️"
                    markup.add(types.InlineKeyboardButton(
                        f"{icon} {perm_label}",
                        callback_data=f"admin_perm_toggle_{edit_tg_id}_{perm_key}",
                        style="success" if has_perm else "primary"
                    ))
                markup.add(types.InlineKeyboardButton(f"{pe('arrow_blue', '🔙')} بازگشت به ادمین‌ها", callback_data="admin_manage_admins", style="danger"))
                _bot.edit_message_text(
                    f"{pe('key', '🔑')} <b>دسترسی‌های ادمین: {name}</b>\n<code>{edit_tg_id}</code>\n\n"
                    "برای فعال/غیرفعال کردن هر بخش روی آن کلیک کنید:\n"
                    f"{pe('checkmark_green', '✅')} = دسترسی دارد | ⬜️ = دسترسی ندارد",
                    chat_id=call.message.chat.id,
                    message_id=call.message.message_id,
                    reply_markup=markup
                )
                ...
                if not admin_info:
                    return _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} ادمین یافت نشد", show_alert=True)
                current_perms = set((admin_info.get("permissions") or "").split(","))
                current_perms.discard("")
                if perm_key in current_perms:
                    current_perms.discard(perm_key)
                    msg = f"{pe('cross_red', '❌')} دسترسی حذف شد"
                else:
                    current_perms.add(perm_key)
                    msg = f"{pe('checkmark_green', '✅')} دسترسی اضافه شد"
                ...
                markup = types.InlineKeyboardMarkup(row_width=1)
                if missions:
                    text = f"{pe('trophy', '🎯')} <b>ماموریت‌های فعال:</b>\n\n"
                    for m in missions:
                        text += f"{pe('arrow_blue', '🔸')} {m['channel_username']} — {pe('diamond', '💎')}{m['reward']} الماس\n"
                        # 🔴 دکمه حذف با رنگ danger (قرمز)
                        markup.add(types.InlineKeyboardButton(f"{pe('cross_red', '❌')} حذف {m['channel_username']}", callback_data=f"del_mission_{m['id']}", style="danger"))
                else:
                    text = f"{pe('edit_box', '📋')} هیچ ماموریتی تعریف نشده.\n\n"
                ...
                _bot.answer_callback_query(call.id, f"{pe('checkmark_green', '✅')} ماموریت حذف شد")
                ...
                markup = types.InlineKeyboardMarkup()
                # 🔴 دکمه لغو با رنگ danger (قرمز)
                markup.add(types.InlineKeyboardButton(f"{pe('cross_red', '❌')} لغو", callback_data="admin_missions", style="danger"))
                _bot.edit_message_text(
                    f"{pe('trophy', '🎯')} <b>افزودن ماموریت</b>\n\nآیدی کانال را ارسال کنید (با @):\nمثال: <code>@mychannel</code>",
                    chat_id=call.message.chat.id, message_id=call.message.message_id,
                ...
                text = f"{pe('game_controller', '🎰')} <b>مدیریت قرعه‌کشی</b>\n\n"
                if active:
                    text += f"{pe('edit_box', '📋')} قرعه‌کشی‌های فعال:\n"
                    for lot in active:
                        status = f"{pe('checkmark_green', '🟢')} فعال" if lot.get("status") == "active" else f"{pe('checkmark_green', '✅')} پایان یافته"
                        text += f"\n• {lot.get('start_time','?')} تا {lot.get('end_time','?')} — {lot.get('winners_count','?')} برنده — {status}"
                        if lot.get("status") == "active":
                            markup.add(types.InlineKeyboardButton(
                                f"{pe('cross_red', '❌')} لغو قرعه‌کشی {lot.get('start_time','?')}",
                ...
                markup.add(types.InlineKeyboardButton(f"{pe('arrow_blue', '🔙')} پنل مدیریت", callback_data="admin_panel", style="danger"))
                _bot.edit_message_text(text, chat_id=call.message.chat.id,
                    message_id=call.message.message_id, reply_markup=markup)
                _bot.answer_callback_query(call.id)
                ...
                markup = types.InlineKeyboardMarkup()
                markup.add(types.InlineKeyboardButton(f"{pe('cross_red', '❌')} لغو", callback_data="admin_lottery", style="danger"))
                _bot.edit_message_text(
                    f"{pe('game_controller', '🎰')} <b>ایجاد قرعه‌کشی</b>\n\n"
                    f"{pe('edit_box', '📝')} <b>مرحله ۱ از ۵: ساعت شروع</b>\n\n"
                ...
                try:
                    ...
                    pass
                _bot.answer_callback_query(call.id, f"{pe('checkmark_green', '✅')} قرعه‌کشی لغو شد", show_alert=True)
                # برگشت به صفحه قرعه‌کشی
                markup = types.InlineKeyboardMarkup(row_width=1)
                markup.add(types.InlineKeyboardButton("➕ ایجاد قرعه‌کشی جدید", callback_data="lottery_create", style="success"))
                markup.add(types.InlineKeyboardButton(f"{pe('arrow_blue', '🔙')} پنل مدیریت", callback_data="admin_panel", style="danger"))
                _bot.edit_message_text(f"{pe('game_controller', '🎰')} <b>مدیریت قرعه‌کشی</b>\n\n{pe('checkmark_green', '✅')} قرعه‌کشی لغو شد.",
                    chat_id=call.message.chat.id, message_id=call.message.message_id, reply_markup=markup)
                ...
                _bot.answer_callback_query(call.id, f"{pe('checkmark_green', '✅')} قرعه‌کشی ثبت شد!", show_alert=True)
                markup = types.InlineKeyboardMarkup()
                markup.add(types.InlineKeyboardButton(f"{pe('game_controller', '🎰')} مدیریت قرعه‌کشی", callback_data="admin_lottery", style="success"))
                markup.add(types.InlineKeyboardButton(f"{pe('arrow_blue', '🔙')} پنل مدیریت", callback_data="admin_panel", style="danger"))
                _bot.edit_message_text(
                    f"{pe('checkmark_green', '✅')} <b>قرعه‌کشی با موفقیت ثبت شد!</b>\n\n"
                    f"{pe('timer_1h', '⏰')} شروع: <b>{lot_data['start_time']}</b>\n"
                    f"{pe('timer_1h', '⏰')} پایان: <b>{lot_data['end_time']}</b>\n"
                    f"{pe('trophy', '🏆')} تعداد برنده: <b>{lot_data['winners_count']} نفر</b>\n"
                    f"{pe('gift', '🎁')} جوایز: {' | '.join(lot_data['prizes'])}\n\n"
                ...
                _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} گزینه نامعتبر")
        
        except Exception as e:
            print(f"{pe('cross_red', '❌')} خطا در callback_admin: {e}")
            try:
                _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} خطا: {str(e)[:100]}", show_alert=True)
            except: 
                pass
    ...
    def handle_owner_state(message):
        try:
            ...
            if state == "broadcast_msg":
                ...
                _bot.reply_to(message, f"{pe('timer_1m', '⏳')} در حال ارسال به {len(tg_ids)} کاربر...")
                sent, failed = 0, 0
                for tid in tg_ids:
                    try:
                        if message.photo:
                        ...
                        failed += 1
                _bot.reply_to(message,
                    f"{pe('checkmark_green', '✅')} ارسال تمام شد!\n\n📤 موفق: {sent}\n{pe('cross_red', '❌')} بلاک‌شده/خطا: {failed}",
            ...
            if state == "waiting_channel":
                ...
                if db.add_forced_channel(text):
                    cache.invalidate("membership_")
                    _bot.reply_to(message, f"{pe('checkmark_green', '✅')} چنل <b>{text}</b> اضافه شد.", reply_markup=_owner_keyboard())
                else:
                    _bot.reply_to(message, f"{pe('warning', '⚠️')} خطا یا تکراری است.", reply_markup=_owner_keyboard())
                _owner_states.pop(message.from_user.id, None)
            
            elif state == "wc_team1":
                state_data["data"]["team1"] = text
                state_data["state"] = "wc_team2"
                _bot.reply_to(message, f"{pe('checkmark_green', '✅')} تیم اول: <b>{text}</b>\n\n{pe('edit_box', '📝')} مرحله  از ۴:\nنام <b>تیم دوم</b> را ارسال کنید:")
            
            elif state == "wc_team2":
                state_data["data"]["team2"] = text
                state_data["state"] = "wc_time"
                _bot.reply_to(message, f"{pe('checkmark_green', '✅')} تیم دوم: <b>{text}</b>\n\n{pe('edit_box', '📝')} مرحله  از ۴:\n ساعت بازی را ارسال کنید:\n\nمثال: <code>20:30</code>")
            
            elif state == "wc_time":
                state_data["data"]["time"] = text
                state_data["state"] = "wc_bet"
                _bot.reply_to(message, f"{pe('checkmark_green', '✅')} ساعت: <b>{text}</b>\n\n{pe('edit_box', '📝')} مرحله ۴ از ۴:\n{pe('diamond', '💎')} مبلغ شرط (الماس) را ارسال کنید:\n\nمثال: <code>10</code>")
            
            elif state == "wc_bet":
                try:
                    bet_amount = int(text)
                except:
                    return _bot.reply_to(message, f"{pe('cross_red', '❌')} مبلغ باید عدد باشد. دوباره تلاش کنید:")
                ...
                # 🔴 دکمه تیم دوم با رنگ danger (قرمز)
                markup.add(
                    types.InlineKeyboardButton(f"{pe('exclamation_red', '🔴')} {data['team2']}", callback_data=f"bet_wc_{challenge_id}_{data['team2']}", style="danger")
                )
                ...
                try:
                    msg = _bot.send_message(group,
                        ...
                        f"{pe('timer_1h', '⏰')} ساعت: <b>{data['time']}</b>\n"
                        f"{pe('diamond', '💎')} مبلغ شرط: <b>{bet_amount} الماس</b>\n\n"
                        f"کدام تیم برنده می‌شود؟ شرط ببندید!",
                        reply_markup=markup)
                    db.update_challenge_message(challenge_id, msg.message_id, msg.chat.id)
                    _bot.reply_to(message, 
                        f"{pe('checkmark_green', '✅')} چالش با موفقیت ایجاد شد!\n\n"
                        f"🆚 {data['team1']} vs {data['team2']}\n"
                        f"{pe('timer_1h', '⏰')} {data['time']} | {pe('diamond', '💎')} {bet_amount}\n"
                        f"{pe('megaphone', '📢')} ID چالش: <code>{challenge_id}</code>",
                        reply_markup=_owner_keyboard())
                except Exception as e:
                    _bot.reply_to(message, f"{pe('cross_red', '❌')} خطا در ارسال به گروه: {e}\nمطمئن شوید ربات در {group} ادمین است.", reply_markup=_owner_keyboard())
                ...
                _bot.reply_to(message, f"{pe('edit_box', '📝')} کاربر: <b>{text}</b>\n\n{pe('diamond', '💎')} مبلغ الماس را ارسال کنید:")
                ...
                to_account = db.get_account_by_username(username)
                if not to_account:
                    _bot.reply_to(message, f"{pe('cross_red', '❌')} کاربر '{username}' یافت نشد.", reply_markup=_owner_keyboard())
                    _owner_states.pop(message.from_user.id, None)
                    return
                ...
                if to_tg_id:
                    try:
                        _bot.send_message(to_tg_id, f"{pe('gift', '🎁')} <b>{amount} الماس</b> از طرف سیستم دریافت کردید!\n{pe('diamond', '💎')} موجودی جدید: <b>{new_balance}</b>")
                    except: 
                        pass
                
                _bot.reply_to(message, 
                    f"{pe('checkmark_green', '✅')} <b>{amount} الماس</b> به <b>{to_account['username']}</b> داده شد.\n{pe('diamond', '💎')} موجودی جدید: <b>{new_balance}</b>",
                    reply_markup=_owner_keyboard())
                _owner_states.pop(message.from_user.id, None)
                ...
                if tg_id:
                    try:
                        _bot.send_message(tg_id, f"{pe('gift', '🎁')} <b>{amount} الماس</b> از طرف مالک دریافت کردید!\n{pe('diamond', '💎')} موجودی جدید: <b>{new_balance}</b>")
                    except: 
                        pass
                
                _bot.reply_to(message, 
                    f"{pe('checkmark_green', '✅')} <b>{amount}</b> الماس به <b>{account['username']}</b> داده شد.\n"
                    f"{pe('diamond', '💎')} موجودی جدید: <b>{new_balance}</b> (معادل {new_balance * token_price} تومان)",
                    reply_markup=_owner_keyboard())
                _owner_states.pop(message.from_user.id, None)
                ...
                try:
                    amount = int(text)
                    if amount <= 0:
                        return _bot.reply_to(message, f"{pe('cross_red', '❌')} تعداد الماس باید بیشتر از صفر باشد. دوباره وارد کنید:")
                except ValueError:
                    return _bot.reply_to(message, f"{pe('cross_red', '❌')} لطفاً یک عدد معتبر وارد کنید:")
                ...
                _bot.reply_to(
                    message,
                    f"{pe('diamond', '💎')} <b>هدیه الماس: {amount} الماس</b>\n\n"
                    "ایدی عددی تلگرام کاربر مورد نظر را وارد کنید:",
                    reply_markup=markup
                )
                ...
                try:
                    ...
                    return _bot.reply_to(message, f"{pe('cross_red', '❌')} ایدی عددی باید فقط شامل اعداد باشد. دوباره وارد کنید:")

                account = db.get_account_by_tg_id(tg_id)
                if not account:
                    return _bot.reply_to(
                        message,
                        f"{pe('cross_red', '❌')} کاربری با این ایدی عددی در سیستم یافت نشد.\n"
                ...
                if gift_type == "diamond":
                    ...
                    gift_desc = f"{pe('edit_box', '📋')} پنل {plan_label} ({days} روز)"
                ...
                confirm_text = (
                    f"{pe('gift', '🎁')} <b>تایید هدیه</b>\n\n"
                    f"{pe('person_star', '👤')} <b>کاربر:</b> {account.get('username', 'نامشخص')}\n"
                    f"🆔 <b>ایدی تلگرام:</b> @{account.get('username', '-')}\n"
                    f"🔢 <b>ایدی عددی:</b> <code>{tg_id}</code>\n"
                    f"{pe('edit_box', '📋')} <b>پلن باقی‌مانده:</b> {plan_remaining}\n"
                    f"{pe('diamond', '💰')} <b>موجودی:</b> {balance} الماس\n"
                    f"{pe('gift', '🎁')} <b>{gift_desc} هدیه</b>\n\n"
                    f"آیا تایید می‌کنید؟"
                )
                markup = types.InlineKeyboardMarkup(row_width=2)
                markup.add(
                    types.InlineKeyboardButton(f"{pe('checkmark_green', '✅')} تایید", callback_data=f"admin_gift_confirm_{gift_key}", style="success"),
                    types.InlineKeyboardButton(f"{pe('cross_red', '❌')} لغو", callback_data="admin_gift_cancel", style="danger")
                )
                _bot.reply_to(message, confirm_text, reply_markup=markup)

            elif state == "gift_awaiting_confirm":
                _bot.reply_to(message, f"{pe('timer_1m', '⏳')} لطفاً روی دکمه تایید یا لغو کلیک کنید.")

            elif state == "set_card":
                card = text.strip().replace("-", "").replace(" ", "")
                db.set_global_setting("card_number", card)
                _bot.reply_to(message,
                    f"{pe('checkmark_green', '✅')} شماره کارت ذخیره شد:\n<code>{card}</code>",
                    reply_markup=_owner_keyboard())
                _owner_states.pop(message.from_user.id, None)
                ...
                try:
                    ...
                    if reward < 1:
                        return _bot.reply_to(message, f"{pe('cross_red', '❌')} جایزه باید بیشتر از ۰ باشد.")
                except ValueError:
                    return _bot.reply_to(message, f"{pe('cross_red', '❌')} مقدار باید عدد باشد.")
                ch = state_data.get("data", {}).get("channel")
                if not ch:
                    _bot.reply_to(message, f"{pe('cross_red', '❌')} خطا: آیدی کانال یافت نشد. دوباره تلاش کنید.", reply_markup=_owner_keyboard())
                    _owner_states.pop(message.from_user.id, None)
                    return
                db.add_mission(ch, reward)
                _bot.reply_to(message,
                    f"{pe('checkmark_green', '✅')} ماموریت اضافه شد!\n{pe('arrow_blue', '🔸')} {ch} — {pe('diamond', '💎')}{reward} الماس",
                    parse_mode="HTML",
                    reply_markup=_owner_keyboard())
                _owner_states.pop(message.from_user.id, None)
                ...
                try:
                    _bot.send_message(
                        new_admin_id,
                        f"{pe('shield', '👮')} <b>شما به عنوان ادمین اضافه شدید!</b>\n\n"
                        "برای دسترسی به پنل مدیریت دستور /subadmin را ارسال کنید."
                    )
                except Exception:
                    pass
                _bot.reply_to(message,
                    f"{pe('checkmark_green', '✅')} <b>{name}</b> (<code>{new_admin_id}</code>) به عنوان ادمین اضافه شد.",
                    reply_markup=_owner_keyboard())
                _owner_states.pop(message.from_user.id, None)
                ...
                if not new_text.strip():
                    return _bot.reply_to(message, f"{pe('cross_red', '❌')} متن نمی‌تواند خالی باشد:")
                # اعتبارسنجی متغیرها
                try:
                    new_text.format(
                    ...
                    return _bot.reply_to(message,
                        f"{pe('cross_red', '❌')} متغیر نامعتبر: <code>{e}</code>\n\n"
                        "متغیرهای مجاز:\n"
                        "<code>{name}</code>  <code>{name_full}</code>  <code>{mention}</code>\n"
                        "<code>{tag}</code>  <code>{tg_id}</code>  <code>{time}</code>\n"
                        "<code>{balance}</code>  <code>{total_earned}</code>  <code>{sub_status}</code>")
                ...
                _owner_states.pop(message.from_user.id, None)
                markup = types.InlineKeyboardMarkup()
                markup.add(types.InlineKeyboardButton(f"{pe('arrow_blue', '🔙')} تنظیمات خوش‌آمد", callback_data="admin_welcome_settings", style="primary"))
                _bot.reply_to(message, f"{pe('checkmark_green', '✅')} متن خوش‌آمد ذخیره شد!", reply_markup=markup)
                ...
                if message.photo:
                    ...
                    return _bot.reply_to(message, f"{pe('cross_red', '❌')} لطفاً یک عکس ارسال کنید:")
                db.set_global_setting("welcome_photo_id", photo_id)
                _owner_states.pop(message.from_user.id, None)
                markup = types.InlineKeyboardMarkup()
                markup.add(types.InlineKeyboardButton(f"{pe('arrow_blue', '🔙')} تنظیمات خوش‌آمد", callback_data="admin_welcome_settings", style="primary"))
                _bot.reply_to(message, f"{pe('checkmark_green', '✅')} عکس خوش‌آمد ذخیره شد!", reply_markup=markup)

            elif state == "guide_name":
                # مرحله ۱: دریافت اسم آموزش
                guide_name = text.strip()
                if not guide_name:
                    return _bot.reply_to(message, f"{pe('cross_red', '❌')} اسم آموزش نمی‌تواند خالی باشد:")
                ...
                markup.add(
                    ...
                    types.InlineKeyboardButton(f"{pe('edit_box', '📝')} ارسال آموزش متنی", callback_data="guide_type_text", style="success")
                )
                markup.add(types.InlineKeyboardButton(f"{pe('cross_red', '❌')} لغو", callback_data="admin_guide_manage", style="danger"))
                _bot.reply_to(message,
                    f"{pe('checkmark_green', '✅')} اسم آموزش: <b>{guide_name}</b>\n\n"
                ...
                if message.video:
                    ...
                    return _bot.reply_to(message, f"{pe('cross_red', '❌')} لطفاً یک ویدیو یا عکس ارسال کنید:")
                ...
                _owner_states.pop(message.from_user.id, None)
                _bot.reply_to(message,
                    f"{pe('checkmark_green', '✅')} آموزش «<b>{guide_name}</b>» ({'ویدیو' if media_type == 'video' else 'عکس'}) ذخیره شد.",
                    reply_markup=_owner_keyboard())
                ...
                if not content.strip():
                    return _bot.reply_to(message, f"{pe('cross_red', '❌')} متن آموزش نمی‌تواند خالی باشد:")
                guide_name = state_data["data"]["name"]
                raw = db.get_global_setting("guide_list", "[]")
                try:
                    guides = _json.loads(raw)
                ...
                _owner_states.pop(message.from_user.id, None)
                _bot.reply_to(message,
                    f"{pe('checkmark_green', '✅')} آموزش متنی «<b>{guide_name}</b>» ذخیره شد.",
                    reply_markup=_owner_keyboard())
            ...
            # ── قرعه‌کشی states ─────────────────────────────────────────────
                ...
                if not _re.match(r"^\d{1,2}:\d{2}$", t):
                    return _bot.reply_to(message, f"{pe('cross_red', '❌')} فرمت اشتباه است.\nمثال: <code>22:00</code>")
                h, m = map(int, t.split(":"))
                if not (0 <= h <= 23 and 0 <= m <= 59):
                    return _bot.reply_to(message, f"{pe('cross_red', '❌')} ساعت نامعتبر است.")
                state_data["data"]["start_time"] = t
                state_data["state"] = "lottery_end_time"
                _bot.reply_to(message,
                    f"{pe('checkmark_green', '✅')} ساعت شروع: <b>{t}</b>\n\n"
                    f"{pe('edit_box', '📝')} <b>مرحله ۲ از ۵: ساعت پایان</b>\n\n"
                ...
                _bot.reply_to(message,
                    f"{pe('checkmark_green', '✅')} ساعت پایان: <b>{t}</b>\n\n"
                    f"{pe('edit_box', '📝')} <b>مرحله ۳ از ۵: تعداد برنده</b>\n\n"
                    "تعداد برندگان را وارد کنید:\n"
                    "مثال: <code>3</code>")

            elif state == "lottery_winners_count":
                if not text.isdigit() or int(text) < 1 or int(text) > 20:
                    return _bot.reply_to(message, f"{pe('cross_red', '❌')} عدد معتبر بین ۱ تا ۲۰ وارد کنید.")
                cnt = int(text)
                ...
                _bot.reply_to(message,
                    f"{pe('checkmark_green', '✅')} تعداد برنده: <b>{cnt} نفر</b>\n\n"
                    f"{pe('edit_box', '📝')} <b>مرحله ۴ از ۵: تعیین جوایز</b>\n\n"
                ...
                prize = text.strip()
                if not prize:
                    return _bot.reply_to(message, f"{pe('cross_red', '❌')} جایزه نمی‌تواند خالی باشد.")
                ...
                if current < total:
                    ...
                    _bot.reply_to(message,
                        f"{pe('checkmark_green', '✅')} جایزه نفر {ordinals[current-1]}: <b>{prize}</b>\n\n"
                    ...
                    markup = types.InlineKeyboardMarkup(row_width=2)
                    markup.add(
                        types.InlineKeyboardButton(f"{pe('checkmark_green', '✅')} تأیید و ثبت", callback_data="lottery_confirm", style="success"),
                        types.InlineKeyboardButton(f"{pe('cross_red', '❌')} لغو", callback_data="admin_lottery", style="danger")
                    )
                    _bot.reply_to(message,
                        f"{pe('edit_box', '📋')} <b>مرحله ۵ از ۵: تأیید قرعه‌کشی</b>\n\n"
                        f"{pe('timer_1h', '⏰')} زمان: <b>{d['start_time']}</b> تا <b>{d['end_time']}</b>\n"
                        f"{pe('trophy', '🏆')} تعداد برنده: <b>{d['winners_count']} نفر</b>\n"
                        f"{pe('gift', '🎁')} جوایز:{prize_text}\n\n"
                        "آیا تأیید می‌کنید؟",
                        reply_markup=markup)
        
        except Exception as e:
            print(f"{pe('cross_red', '❌')} خطا در handle_owner_state: {e}")
            _bot.reply_to(message, f"{pe('cross_red', '❌')} خطا: {e}", reply_markup=_owner_keyboard())
            _owner_states.pop(message.from_user.id, None)
    ...
    def cmd_text_commands(message):
        if message.from_user.id != OWNER_TG_ID:
            return
        _bot.reply_to(message, 
            f"{pe('megaphone', '📢')} تمام دستورات مدیریتی به پنل دکمه‌ای منتقل شدند.\n\n"
            f"روی دکمه <b>{pe('megaphone', '📢')} مدیریت</b> کلیک کنید.",
            reply_markup=_owner_keyboard())
    ...
    def _subadmin_panel_keyboard():
        ...
        markup = types.InlineKeyboardMarkup(row_width=2)
        markup.add(
            types.InlineKeyboardButton(f"{pe('group_add', '👥')} کاربران", callback_data="sa_users", style="primary"),
            types.InlineKeyboardButton(f"{pe('calendar', '📅')} بازی‌های امروز", callback_data="sa_today_games", style="primary")
        )
        markup.add(
            types.InlineKeyboardButton(f"{pe('megaphone', '📣')} پیام عمومی", callback_data="sa_broadcast", style="primary"),
            types.InlineKeyboardButton(f"{pe('trophy', '🎯')} ماموریت‌ها", callback_data="sa_missions", style="success")
        )
        markup.add(
            types.InlineKeyboardButton(f"{pe('group_add', '👥')} شرکت‌کنندگان جام جهانی", callback_data="sa_wc_participants", style="primary")
        )
        markup.add(
            types.InlineKeyboardButton(f"{pe('gift', '🎁')} هدیه", callback_data="sa_gift", style="success")
        )
        markup.add(
            types.InlineKeyboardButton(f"{pe('book', '📚')} مدیریت راهنما", callback_data="sa_guide_manage", style="primary")
        )
        return markup
    ...
    def cmd_subadmin_panel(message):
        ...
        if not db.is_sub_admin(tg_id):
            return _bot.reply_to(message, f"{pe('cross_red', '❌')} شما دسترسی ادمین ندارید.")
        _bot.reply_to(message,
            f"{pe('shield', '👮')} <b>پنل مدیریت ادمین</b>\n\nیکی از گزینه‌های زیر را انتخاب کنید:",
    ...
    def callback_subadmin(call):
        ...
        try:
            ...
            if data == "sa_users":
                accounts = db.get_all_accounts()
                lines = [f"{pe('group_add', '👥')} <b>لیست کاربران ({len(accounts)} نفر):</b>\n"]
                ...
                markup = types.InlineKeyboardMarkup()
                markup.add(types.InlineKeyboardButton(f"{pe('arrow_blue', '🔙')} بازگشت", callback_data="sa_back", style="danger"))
                _bot.edit_message_text("\n".join(lines),
                    chat_id=call.message.chat.id, message_id=call.message.message_id, reply_markup=markup)
                _bot.answer_callback_query(call.id)
            ...
            # ── بازی‌های امروز ───────────────────────────────────────────────
                ...
                if not challenges:
                    text = f"{pe('calendar', '📅')} <b>بازی‌های امروز</b>\n\nهیچ بازی‌ای برای امروز ثبت نشده."
                else:
                    lines = [f"{pe('calendar', '📅')} <b>بازی‌های امروز ({len(challenges)} بازی):</b>\n"]
                    for c in challenges:
            ...
            # ── پیام عمومی ───────────────────────────────────────────────────
                ...
                markup = types.InlineKeyboardMarkup()
                markup.add(types.InlineKeyboardButton(f"{pe('cross_red', '❌')} لغو", callback_data="sa_back", style="danger"))
                _bot.edit_message_text(
            ...
            # ── ماموریت‌ها ───────────────────────────────────────────────────
                ...
                if missions:
                    ...
                    for m in missions:
                        text += f"{pe('arrow_blue', '🔸')} {m['channel_username']} — {pe('diamond', '💎')}{m['reward']} الماس\n"
                        markup.add(types.InlineKeyboardButton(
                            f"{pe('cross_red', '❌')} حذف {m['channel_username']}", callback_data=f"sa_del_mission_{m['id']}", style="danger"))
                else:
                    text = f"{pe('edit_box', '📋')} هیچ ماموریتی تعریف نشده.\n\n"
                ...
                markup = types.InlineKeyboardMarkup()
                markup.add(types.InlineKeyboardButton(f"{pe('cross_red', '❌')} لغو", callback_data="sa_missions", style="danger"))
                _bot.edit_message_text(
            ...
            # ── هدیه ─────────────────────────────────────────────────────────
                ...
                markup = types.InlineKeyboardMarkup(row_width=2)
                markup.add(
                    types.InlineKeyboardButton(f"{pe('diamond', '💎')} الماس", callback_data="sa_gift_diamond", style="primary"),
                    types.InlineKeyboardButton(f"{pe('edit_box', '📋')} پنل", callback_data="sa_gift_panel", style="success")
                )
                markup.add(types.InlineKeyboardButton(f"{pe('cross_red', '❌')} لغو", callback_data="sa_back", style="danger"))
                _bot.edit_message_text(
                ...
                markup = types.InlineKeyboardMarkup(row_width=1)
                markup.add(
                    types.InlineKeyboardButton(f"{pe('calendar', '📅')} پنل یک ماهه (30 روز)", callback_data="sa_gift_plan_30", style="primary"),
                    types.InlineKeyboardButton(f"{pe('calendar', '📅')} پنل یک هفته‌ای (7 روز)", callback_data="sa_gift_plan_7", style="primary"),
                    types.InlineKeyboardButton(f"{pe('calendar', '📅')} پنل یک روزه (1 روز)", callback_data="sa_gift_plan_1", style="primary")
                )
                markup.add(types.InlineKeyboardButton(f"{pe('cross_red', '❌')} لغو", callback_data="sa_back", style="danger"))
                _bot.edit_message_text(
                    f"{pe('edit_box', '📋')} <b>هدیه پنل</b>\n\nنوع پنل هدیه را انتخاب کنید:",
                ...
                if not gift_info or gift_info.get("key") != gift_key:
                    _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} اطلاعات منقضی شده.", show_alert=True)
                    return
                ...
                if gift_type == "diamond":
                    ...
                    try:
                        ...
                        pass
                    result_text = f"{pe('checkmark_green', '✅')} <b>{amount} الماس</b> به <b>{account['username']}</b> هدیه داده شد."
                else:
                    days = gift_info["days"]
                    plan_label = gift_info["plan_label"]
                    db.set_subscription(account["id"], "gift", days)
                    try:
                        ...
                        pass
                    result_text = f"{pe('checkmark_green', '✅')} پنل <b>{plan_label}</b> به <b>{account['username']}</b> هدیه داده شد."
                _subadmin_states.pop(tg_id, None)
                markup = types.InlineKeyboardMarkup()
                markup.add(types.InlineKeyboardButton(f"{pe('arrow_blue', '🔙')} پنل مدیریت", callback_data="sa_back", style="primary"))
                _bot.edit_message_text(result_text,
                    chat_id=call.message.chat.id, message_id=call.message.message_id, reply_markup=markup)
                _bot.answer_callback_query(call.id, f"{pe('checkmark_green', '✅')} هدیه ارسال شد!")

            elif data == "sa_gift_cancel":
                _subadmin_states.pop(tg_id, None)
                markup = _subadmin_panel_keyboard()
                _bot.edit_message_text(f"{pe('cross_red', '❌')} عملیات هدیه لغو شد.",
                    chat_id=call.message.chat.id, message_id=call.message.message_id, reply_markup=markup)
                _bot.answer_callback_query(call.id)
            ...
            # ── بازگشت ───────────────────────────────────────────────────────
                ...
                if guides:
                    ...
                    for i, g in enumerate(guides):
                        txt += f"{'🎥' if g['type'] == 'video' else '🖼' if g['type'] == 'photo' else '📝'} {g['name']}\n"
                        markup.add(types.InlineKeyboardButton(
                            f"{pe('cross_red', '❌')} حذف «{g['name']}»", callback_data=f"sa_guide_del_{i}", style="danger"))
                else:
                    txt = f"{pe('book', '📚')} <b>مدیریت راهنما</b>\n\nهیچ آموزشی ثبت نشده."
                ...
                markup = types.InlineKeyboardMarkup()
                markup.add(types.InlineKeyboardButton(f"{pe('cross_red', '❌')} لغو", callback_data="sa_guide_manage", style="danger"))
                _bot.edit_message_text(
                    f"{pe('book', '📚')} <b>افزودن راهنما</b>\n\nاسم آموزش را وارد کنید:",
                    chat_id=call.message.chat.id, message_id=call.message.message_id, reply_markup=markup)
                _bot.answer_callback_query(call.id)
                ...
                if guides:
                    ...
                    txt = f"{pe('book', '📚')} هیچ آموزشی ثبت نشده."
                markup.add(types.InlineKeyboardButton("➕ اضافه کردن راهنما", callback_data="sa_guide_add", style="success"))
                markup.add(types.InlineKeyboardButton(f"{pe('arrow_blue', '🔙')} بازگشت", callback_data="sa_back", style="danger"))
                _bot.edit_message_text(txt, chat_id=call.message.chat.id,
                    message_id=call.message.message_id, reply_markup=markup)
            ...
            print(f"{pe('cross_red', '❌')} خطا در callback_subadmin: {e}")
            _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} خطا: {e}", show_alert=True)
    ...
    def handle_subadmin_state(message):
        ...
        try:
            ...
            if state == "sa_broadcast_msg":
                ...
                for tid in tg_ids:
                    try:
                        ...
                        failed += 1
                _bot.reply_to(message, f"{pe('checkmark_green', '✅')} ارسال تمام شد!\n📤 موفق: {sent}\n{pe('cross_red', '❌')} خطا: {failed}",
                    reply_markup=_subadmin_panel_keyboard())
                ...
                _bot.reply_to(message, f"{pe('checkmark_green', '✅')} کانال: <b>{ch}</b>\n\n{pe('diamond', '💎')} مقدار جایزه (الماس) را وارد کنید:")
                ...
                db.add_mission(ch, reward)
                _subadmin_states.pop(tg_id, None)
                _bot.reply_to(message, f"{pe('checkmark_green', '✅')} ماموریت اضافه شد!\n{pe('arrow_blue', '🔸')} {ch} — {pe('diamond', '💎')}{reward} الماس",
                    reply_markup=_subadmin_panel_keyboard())
                ...
                try:
                    amount = int(text)
                    if amount <= 0:
                        return _bot.reply_to(message, f"{pe('cross_red', '❌')} تعداد باید بیشتر از صفر باشد:")
                except ValueError:
                    return _bot.reply_to(message, f"{pe('cross_red', '❌')} عدد معتبر وارد کنید:")
                state_data["data"]["amount"] = amount
                ...
                try:
                    ...
                    return _bot.reply_to(message, f"{pe('cross_red', '❌')} ایدی عددی باید فقط شامل اعداد باشد:")
                account = db.get_account_by_tg_id(target_tg_id)
                if not account:
                    return _bot.reply_to(message, f"{pe('cross_red', '❌')} کاربری با این ایدی یافت نشد.")
                gift_type = state_data["data"]["gift_type"]
                balance = db.get_token_balance(account["id"])
                sub = db.get_subscription(account["id"])
                ...
                markup = types.InlineKeyboardMarkup(row_width=2)
                markup.add(
                    types.InlineKeyboardButton(f"{pe('checkmark_green', '✅')} تایید", callback_data=f"sa_gift_confirm_{gift_key}", style="success"),
                    types.InlineKeyboardButton(f"{pe('cross_red', '❌')} لغو", callback_data="sa_gift_cancel", style="danger")
                )
                _bot.reply_to(message, confirm_text, reply_markup=markup)
            ...
            print(f"{pe('cross_red', '❌')} خطا در handle_subadmin_state: {e}")
            _bot.reply_to(message, f"{pe('cross_red', '❌')} خطا: {e}")
            _subadmin_states.pop(tg_id, None)
    ...
    # 🎯 سیستم ماموریت‌ها
    # ══════════════════════════════════════════════════════════════════════════
    @_bot.message_handler(func=lambda m: m.text == f"{pe('trophy', '🎯')} ماموریت‌ها", chat_types=['private'])
    def cmd_missions(message):
        try:
            ...
            print(f"{pe('cross_red', '❌')} خطا در cmd_missions: {e}")
    ...
    # 📖 سیستم راهنما
    # ══════════════════════════════════════════════════════════════════════════
    @_bot.message_handler(func=lambda m: m.text == f"{pe('book', '📖')} راهنما", chat_types=['private'])
    ...
    def callback_guide(call):
        ...
        if data == "guide_menu":
            ...
            if idx < 0 or idx >= len(guides):
                return _bot.send_message(chat_id, f"{pe('cross_red', '❌')} این آموزش یافت نشد.")
            g = guides[idx]
            back_markup = types.InlineKeyboardMarkup()
            back_markup.add(types.InlineKeyboardButton(f"{pe('arrow_blue', '🔙')} بازگشت به راهنما", callback_data="guide_menu", style="primary"))
            try:
                if g["type"] == "video":
                    ...
                    _bot.send_message(chat_id,
                        f"{pe('edit_box', '📝')} <b>{g['name']}</b>\n\n{g.get('content', '')}",
                        reply_markup=back_markup)
            except Exception as e:
                _bot.send_message(chat_id, f"{pe('cross_red', '❌')} خطا در نمایش آموزش: {e}")
    ...
    def _show_guide_menu(tg_id, chat_id):
        ...
        if not guides:
            ...
            return _bot.send_message(chat_id,
                f"{pe('book', '📚')} <b>راهنما</b>\n\nهنوز هیچ آموزشی اضافه نشده.",
                reply_markup=_main_inline_keyboard(account))
        markup = types.InlineKeyboardMarkup(row_width=1)
        for i, g in enumerate(guides):
            icon = "🎥" if g["type"] == "video" else "🖼" if g["type"] == "photo" else f"{pe('edit_box', '📝')}"
            markup.add(types.InlineKeyboardButton(
                f"{icon} {g['name']}", callback_data=f"guide_view_{i}", style="primary"))
        account = _get_account_cached(tg_id)
        if account:
            markup.add(types.InlineKeyboardButton(f"{pe('arrow_blue', '🔙')} منوی اصلی", callback_data="back_main", style="danger"))
        _bot.send_message(chat_id,
    ...
    def _do_missions(tg_id, chat_id):
        try:
            ...
            markup = types.InlineKeyboardMarkup(row_width=1)
            lines = [f"{pe('trophy', '🎯')} <b>ماموریت‌ها</b>\n\nبرای دریافت جایزه، در کانال‌های زیر عضو شوید:\n"]
            for m in missions:
                done = m["id"] in completed_ids
                status = f"{pe('checkmark_green', '✅')}" if done else f"{pe('timer_1m', '⏳')}"
                ch_clean = m["channel_username"].lstrip("@")
                lines.append(f"{status} {m['channel_username']} — {pe('diamond', '💎')}{m['reward']} الماس")
                if not done:
                    # 🔵 دکمه عضویت با رنگ primary (آبی)
                    markup.add(types.InlineKeyboardButton(
                        f"{pe('link_chain', '🔗')} عضویت در {m['channel_username']}",
                        url=f"https://t.me/{ch_clean}",
                        style="primary"
                    ))
            # 🟢 دکمه بررسی با رنگ success (سبز)
            markup.add(types.InlineKeyboardButton(f"{pe('checkmark_green', '✅')} بررسی و دریافت جایزه", callback_data="check_missions", style="success"))
            _bot.send_message(chat_id, "\n".join(lines), reply_markup=markup)
        except Exception as e:
            print(f"{pe('cross_red', '❌')} خطا در _do_missions: {e}")
    ...
    def callback_check_missions(call):
        try:
            ...
            if not newly_done:
                ...
                if not pending:
                    return _bot.answer_callback_query(call.id, f"{pe('checkmark_green', '✅')} همه ماموریت‌ها قبلاً انجام شده!", show_alert=True)
                return _bot.answer_callback_query(call.id,
                    f"{pe('cross_red', '❌')} ماموریت انجام نشده!\nابتدا در {len(pending)} کانال عضو شوید سپس دوباره بررسی کنید.",
                    show_alert=True)

            cache.invalidate(f"account_{call.from_user.id}")
            new_balance = db.get_token_balance(account["id"])
            _bot.answer_callback_query(call.id,
                f"{pe('gift', '🎉')} تبریک! {len(newly_done)} ماموریت انجام شد!\n{pe('diamond', '💎')} +{total_reward} الماس دریافت کردید!",
                show_alert=True)
        except Exception as e:
            print(f"{pe('cross_red', '❌')} خطا در callback_check_missions: {e}")
            _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} خطا: {str(e)[:80]}", show_alert=True)
    ...
    def cmd_unknown(message):
        try:
            ...
            _bot.reply_to(message, f"{pe('warning', '⚠️')} دستور نامعتبر. از دکمه‌های زیر استفاده کنید:", reply_markup=kb)
        except Exception as e:
            print(f"{pe('cross_red', '❌')} خطا در cmd_unknown: {e}")
    ...
    def _hokm_lobby_text(g):
        ...
        return (
            f"🎮 <b>بازی حکم</b>\n"
            f"{pe('person_star', '👤')} سازنده: {g['creator_name']}\n"
            f"{pe('diamond', '💰')} شرط: <b>{g['bet']} الماس</b>\n"
            f"{pe('group_add', '👥')} بازیکنان ({len(players)}/4):\n{names}\n\n"
    ...
    def _hokm_lobby_kb(game_id):
        ...
        kb.add(
            ...
            types.InlineKeyboardButton(f"{pe('cross_red', '❌')} لغو",         callback_data=f"hokm_cancel_{game_id}"),
        )
        return kb
    ...
    def cmd_hokm_start(message):
        try:
            ...
            if bet < 1:
                return _bot.reply_to(message, f"{pe('cross_red', '❌')} مبلغ شرط باید بیشتر از صفر باشد.")
            ...
            if existing:
                return _bot.reply_to(message, f"{pe('warning', '⚠️')} شما در حال حاضر در یک بازی حکم هستید!")
            ...
            print(f"{pe('cross_red', '❌')} cmd_hokm_start: {e}")
    ...
    def callback_hokm_join(call):
        try:
            ...
            if not game:
                return _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} بازی یافت نشد یا تمام شده.")

            if game["state"] != "lobby":
                return _bot.answer_callback_query(call.id, f"{pe('timer_1m', '⏳')} بازی در حال اجراست.")
            ...
            if user_id in game["players"]:
                ...
                _bot.answer_callback_query(call.id, f"{pe('checkmark_green', '✅')} از بازی خارج شدید.")
            else:
                # ورود
                if len(game["players"]) >= 4:
                    return _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} بازی پر است.")
                balance = db.get_token_balance(account["id"])
                if balance < game["bet"]:
                    return _bot.answer_callback_query(call.id,
                        f"{pe('cross_red', '❌')} موجودی کافی ندارید! نیاز: {game['bet']} الماس", show_alert=True)
                ...
                _bot.answer_callback_query(call.id, f"{pe('checkmark_green', '✅')} وارد بازی شدید!")
            ...
            print(f"{pe('cross_red', '❌')} callback_hokm_join: {e}")
    ...
    def callback_hokm_cancel(call):
        try:
            ...
            if not game:
                return _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} بازی یافت نشد.")
            if call.from_user.id != game["creator_id"]:
                return _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} فقط سازنده می‌تواند لغو کند.", show_alert=True)
            if game["state"] != "lobby":
                return _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} بازی شروع شده، نمی‌توان لغو کرد.", show_alert=True)
            ...
            _bot.edit_message_text(
                f"{pe('cross_red', '❌')} بازی حکم لغو شد.",
                chat_id, call.message.message_id
            )
            _bot.answer_callback_query(call.id, f"{pe('checkmark_green', '✅')} بازی لغو شد.")
        except Exception as e:
            print(f"{pe('cross_red', '❌')} callback_hokm_cancel: {e}")
    ...
    def _hokm_send_or_edit(uid, game, text, kb=None):
        ...
        try:
            ...
            print(f"{pe('warning', '⚠️')} _hokm_send_or_edit {uid}: {e}")
    ...
    def _hokm_status_lines(game, uid):
        ...
        if game.get("state") == "playing":
            t0 = game["tricks"][0]
            t1 = game["tricks"][1]
            lines.append(f"{pe('lines', '📊')} تیم حاکم: {t0} | تیم رقیب: {t1}")
            ...
            if game.get("turn_order"):
                ...
                lines.append(f"{pe('trophy', '🎯')} نوبت: <b>{who}</b>")
        return lines
    ...
    def _hokm_send_hand(uid, game, extra_lines=None, extra_rows=None):
        ...
        try:
            ...
            print(f"{pe('warning', '⚠️')} _hokm_send_hand {uid}: {e}")
    ...
    def _hokm_begin(game_id):
        try:
            ...
            try:
                _bot.edit_message_text(
                    f"{pe('checkmark_green', '✅')} <b>بازی حکم شروع شد!</b>\n\n"
                    f"{pe('group_add', '👥')} بازیکنان:\n{names_list}\n\n"
            ...
            for uid in players:
                ...
                text = (
                    ...
                    f"{pe('game_controller', '🎲')} کارت قرعه شما: <b>{card_label}</b>\n"
                    f"{pe('crown', '👑')} حاکم: <b>{hakem_name}</b>{'  ← شما!' if is_hakem else ''}\n\n"
                    f"برای ادامه دکمه زیر را بزنید:"
                )
                _hokm_send_or_edit(uid, game, text, kb)

        except Exception as e:
            print(f"{pe('cross_red', '❌')} _hokm_begin: {e}")
    ...
    def callback_hokm_ready(call):
        try:
            ...
            _bot.answer_callback_query(call.id, f"{pe('checkmark_green', '✅')} آماده‌اید!")
            try:
                _bot.edit_message_text(
                    f"{pe('checkmark_green', '✅')} <b>آماده‌اید!</b>\nمنتظر بقیه بازیکنان بمانید...",
            ...
            print(f"{pe('cross_red', '❌')} callback_hokm_ready: {e}")
        ...
        try:
            ...
            for uid in game["players"]:
                if uid == hakem:
                    ...
                    _hokm_send_hand(
                        uid, game,
                        extra_lines=[f"{pe('crown', '👑')} شما حاکم هستید! حکم را انتخاب کنید:"],
                        extra_rows=trump_rows
                    )
                else:
                    _hokm_send_hand(uid, game, extra_lines=[f"{pe('timer_1m', '⏳')} منتظر انتخاب حکم توسط حاکم..."])

        except Exception as e:
            print(f"{pe('cross_red', '❌')} _hokm_deal_initial: {e}")
    ...
    def callback_hokm_trump(call):
        try:
            ...
            if user_id != game["hakem"]:
                return _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} فقط حاکم می‌تواند حکم انتخاب کند.", show_alert=True)
            if game["state"] != "pick_trump":
                return _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} زمان انتخاب حکم تمام شده.")

            game["trump"] = suit
            suit_label = _SUIT_NAMES[suit]
            _bot.answer_callback_query(call.id, f"{pe('checkmark_green', '✅')} حکم {suit_label} انتخاب شد!")

            # پخش کامل کارت‌ها (پیام شخصی هر بازیکن همین‌جا ادیت می‌شود و حکم انتخابی را نشان می‌دهد)
            _hokm_deal_full(game_id)

        except Exception as e:
            print(f"{pe('cross_red', '❌')} callback_hokm_trump: {e}")
    ...
    def _hokm_deal_full(game_id):
        try:
            ...
            print(f"{pe('cross_red', '❌')} _hokm_deal_full: {e}")
    ...
    def callback_hokm_play(call):
        try:
            ...
            if game["state"] != "playing":
                return _bot.answer_callback_query(call.id, f"{pe('timer_1m', '⏳')} بازی در حال راه‌اندازی است.")
            if user_id not in game["players"]:
                return _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} شما در این بازی نیستید.")

            current_uid = game["turn_order"][game["current_turn_idx"]]
            if user_id != current_uid:
                return _bot.answer_callback_query(call.id, f"{pe('timer_1m', '⏳')} نوبت شما نیست!", show_alert=True)

            hand = game["hands"].get(user_id, [])
            if card_idx >= len(hand):
                return _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} کارت نامعتبر.")
            ...
            if game["lead_suit"] and card["suit"] != game["lead_suit"]:
                ...
                if has_lead:
                    return _bot.answer_callback_query(
                        call.id,
                        f"{pe('cross_red', '❌')} باید از خال {_SUIT_EMOJI.get(game['lead_suit'],'')} پیروی کنید!",
                        show_alert=True
                    )
            ...
            _bot.answer_callback_query(call.id, f"{pe('checkmark_green', '✅')} {_hokm_card_label(card)} بازی شد.")
            ...
            print(f"{pe('cross_red', '❌')} callback_hokm_play: {e}")
    ...
    def _hokm_resolve_round(game_id):
        try:
            ...
            round_summary = [
                ...
                f"{pe('trophy', '🏆')} برنده: <b>{winner_name}</b>",
            ]
            ...
            print(f"{pe('cross_red', '❌')} _hokm_resolve_round: {e}")
    ...
    def _hokm_finish(game_id, team0_won: bool, round_summary=None):
        try:
            ...
            if round_summary:
                ...
                result_lines.append("")
            result_lines.extend([
                f"{pe('trophy', '🏆')} <b>بازی حکم تمام شد!</b>",
                "",
                f"{'🥇 تیم حاکم' if team0_won else '🥇 تیم رقیب'} برنده شد!",
                "",
                f"{pe('checkmark_green', '✅')} برندگان: <b>{win_names}</b>",
                f"{pe('cross_red', '❌')} بازندگان: {lose_names}",
                "",
                f"{pe('diamond', '💰')} مجموع شرط: {total} الماس",
                f"🏛 مالیات ۱۰٪: {tax} الماس",
                f"{pe('diamond', '💎')} هر برنده: <b>{payout_each} الماس</b>",
                "",
                f"{pe('lines', '📊')} نتیجه: تیم حاکم {game['tricks'][0]} — تیم رقیب {game['tricks'][1]}",
            ])
            result_text = "\n".join(result_lines)
            ...
            print(f"{pe('cross_red', '❌')} _hokm_finish: {e}")
    ...
    def _rps_status_text(game):
        ...
        if state == "waiting":
            return (
                ...
                f"{pe('person_star', '👤')} نفر اول: <b>{p1}</b>\n"
                f"{pe('person_star', '👤')} نفر دوم: در انتظار...\n\n"
                f"{pe('diamond', '💰')} شرط هر نفر: <b>{bet} {pe('diamond', '💎')} الماس</b>\n"
                f"{pe('trophy', '🏆')} جایزه برنده: <b>{payout} {pe('diamond', '💎')} الماس</b> (مالیات ۱۰٪)\n\n"
        ...
        if state == "finished":
            ...
            c1_done = f"{pe('checkmark_green', '✅')}" if game.get("choice1") else f"{pe('timer_1m', '⏳')}"
            c2_done = f"{pe('checkmark_green', '✅')}" if game.get("choice2") else f"{pe('timer_1m', '⏳')}"
            lines.append(f"🎮 <b>سنگ کاغذ قیچی — راند {rnd} از {_RPS_ROUNDS}</b>")
            lines.append(f"{pe('person_star', '👤')} {p1}  {c1_done}")
            lines.append(f"{pe('person_star', '👤')} {p2}  {c2_done}")
        ...
        lines.append("")
        lines.append(f"{pe('lines', '📊')} امتیاز: {p1} <b>{s1}</b> — <b>{s2}</b> {p2}")
        lines.append(bar)

        if state == "playing":
            lines.append("")
            lines.append(f"{pe('diamond', '💰')} شرط: <b>{bet} {pe('diamond', '💎')}</b> هر نفر")
    ...
    def _rps_join_markup(game_id, bet):
        markup = types.InlineKeyboardMarkup(row_width=1)
        markup.add(types.InlineKeyboardButton(
            f"⚔️ ورود به بازی — {bet} {pe('diamond', '💎')} الماس",
    ...
    def _rps_render(game_id):
        ...
        try:
            ...
            print(f"{pe('cross_red', '❌')} _rps_render: {e}")
    ...
    def cmd_rps_start(message):
        try:
            ...
            if bet <= 0:
                return _bot.reply_to(message, f"{pe('cross_red', '❌')} مقدار شرط باید بیشتر از صفر باشد.")
            ...
            def _rps_timeout(gid):
                ...
                try:
                    _bot.edit_message_text(
                        f"{pe('timer_1h', '⏰')} <b>بازی لغو شد!</b>\n\n{g['player1_name']} منتظر حریف ماند ولی کسی نیامد.\n"
                        f"{pe('diamond', '💎')} {g['bet']} الماس به حساب برگشت.",
                        g["chat_id"], g["msg_id"],
                        parse_mode="HTML",
            ...
            print(f"{pe('cross_red', '❌')} cmd_rps_start: {e}")
        ...
        try:
            ...
            with _rps_lock:
                ...
                if not game:
                    return _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} بازی یافت نشد.")
                if game["state"] != "waiting":
                    return _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} بازی قبلاً شروع شده.", show_alert=True)
                if user.id == game["player1"]:
                    return _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} شما سازنده این بازی هستید!", show_alert=True)
                ...
                for gid, g in _rps_games.items():
                    if gid == game_id:
                        continue
                    if user.id in (g["player1"], g.get("player2")) and g["state"] != "finished":
                        return _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} شما در یک بازی دیگر هستید.", show_alert=True)

                # کسر الماس از نفر دوم
                if not db.deduct_tokens(account["id"], bet):
                    return _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} خطا در کسر موجودی.", show_alert=True)
                cache.invalidate(f"account_{user.id}")
            ...
            _bot.answer_callback_query(call.id, f"{pe('checkmark_green', '✅')} وارد بازی شدید! {bet} الماس کسر شد.")
            _rps_render(game_id)

        except Exception as e:
            print(f"{pe('cross_red', '❌')} callback_rps_join: {e}")
        ...
        try:
            ...
            with _rps_lock:
                ...
                if not game:
                    return _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} بازی یافت نشد.")
                if game["state"] != "playing":
                    return _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} بازی در جریان نیست.")

                if user.id == game["player1"]:
                    if game["choice1"]:
                        return _bot.answer_callback_query(call.id, f"{pe('warning', '⚠️')} شما قبلاً انتخاب کردید.", show_alert=True)
                    game["choice1"] = choice
                    ...
                    return _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} شما در این بازی نیستید.", show_alert=True)

                _bot.answer_callback_query(call.id, f"{pe('checkmark_green', '✅')} {_RPS_CHOICES.get(choice, choice)} انتخاب شد! منتظر رقیب...")
            ...
            print(f"{pe('cross_red', '❌')} callback_rps_pick: {e}")
    ...
    def _rps_resolve_round(game_id):
        ...
        try:
            ...
            with _rps_lock:
                ...
                if result == "win1":
                    round_result_line = f"{pe('trophy', '🏅')} {p1} این راند را برد!"
                elif result == "win2":
                    round_result_line = f"{pe('trophy', '🏅')} {p2} این راند را برد!"
                else:
                    round_result_line = f"{pe('person_check', '🤝')} این راند مساوی شد!"

                game["last_round_line"] = (
                    f"{pe('edit_box', '📋')} راند {rnd}: {p1}={label1} | {p2}={label2}\n{round_result_line}"
                )
            ...
            print(f"{pe('cross_red', '❌')} _rps_resolve_round: {e}")
    ...
    def _rps_finish(game_id):
        ...
        try:
            ...
            if s1 > s2:
                ...
                winner_name = p1_name
                result_line = f"{pe('trophy', '🏆')} <b>{winner_name}</b> برنده شد! ({s1} — {s2})"
                payout_line = (
                    f"{pe('diamond', '💰')} مجموع شرط: {total} {pe('diamond', '💎')}\n"
                    f"🏛 مالیات ۱۰٪: {tax} {pe('diamond', '💎')}\n"
                    f"{pe('diamond', '💎')} جایزه: <b>{payout} الماس</b> به {winner_name}"
                )
            elif s2 > s1:
                db.add_tokens(game["account2"], payout)
                cache.invalidate(f"account_{game['player2']}")
                winner_name = p2_name
                result_line = f"{pe('trophy', '🏆')} <b>{winner_name}</b> برنده شد! ({s2} — {s1})"
                payout_line = (
                    f"{pe('diamond', '💰')} مجموع شرط: {total} {pe('diamond', '💎')}\n"
                    f"🏛 مالیات ۱۰٪: {tax} {pe('diamond', '💎')}\n"
                    f"{pe('diamond', '💎')} جایزه: <b>{payout} الماس</b> به {winner_name}"
                ...
                cache.invalidate(f"account_{game['player1']}")
                cache.invalidate(f"account_{game['player2']}")
                result_line = f"{pe('person_check', '🤝')} <b>مساوی!</b> ({s1} — {s2})"
                payout_line = f"↩️ هر نفر {bet} {pe('diamond', '💎')} الماس دریافت کرد."

            bar = _rps_score_bar(s1, s2)
            final_text = (
                "🏁 <b>نتیجه نهایی — سنگ کاغذ قیچی</b>\n\n"
                f"{pe('person_star', '👤')} {p1_name}:  <b>{s1}</b> امتیاز\n"
                f"{pe('person_star', '👤')} {p2_name}:  <b>{s2}</b> امتیاز\n"
                f"{bar}\n\n"
                f"{result_line}\n\n"
                f"{payout_line}"
            )
            ...
            print(f"{pe('cross_red', '❌')} _rps_finish: {e}")
                ...
                for lot in lotteries:
                    ...
                    if not lot.get("announced") and now_teh >= start_dt:
                        try:
                            ...
                            for i, p in enumerate(prizes):
                                ord_name = ordinals[i] if i < len(ordinals) else f"{i+1}م"
                                medals = ["🥇","🥈","🥉",f"{pe('trophy', '🏅')}",f"{pe('trophy', '🏅')}",f"{pe('trophy', '🏅')}",f"{pe('trophy', '🏅')}",f"{pe('trophy', '🏅')}",f"{pe('trophy', '🏅')}",f"{pe('trophy', '🏅')}"]
                                medal = medals[i] if i < len(medals) else f"{pe('gift', '🎁')}"
                                prize_lines += f"\n{medal} نفر {ord_name}: <b>{p}</b>"

                            msg_text = (
                                f"{pe('game_controller', '🎰')} <b>قرعه‌کشی شروع شد!</b>\n\n"
                                f"{pe('timer_1h', '⏰')} مهلت شرکت: تا ساعت <b>{lot['end_time']}</b>\n"
                                f"{pe('trophy', '🏆')} تعداد برنده: <b>{lot['winners_count']} نفر</b>\n"
                                f"{pe('gift', '🎁')} جوایز:{prize_lines}\n\n"
                                "👇 برای شرکت روی دکمه کلیک کنید:"
                            )
                            markup = types.InlineKeyboardMarkup()
                            markup.add(types.InlineKeyboardButton(
                                f"{pe('game_controller', '🎰')} شرکت در قرعه‌کشی",
                            ...
                            changed = True
                            print(f"{pe('checkmark_green', '✅')} قرعه‌کشی {lot['id']} اعلام شد")
                        except Exception as e:
                            print(f"{pe('cross_red', '❌')} خطا در اعلام قرعه‌کشی: {e}")
                    ...
                    if lot.get("announced") and not lot.get("finished") and now_teh >= end_dt:
                        try:
                            ...
                            if not participants:
                                result_text = (
                                    f"{pe('game_controller', '🎰')} <b>نتیجه قرعه‌کشی</b>\n\n"
                                ...
                                result_text = (
                                    f"{pe('gift', '🎉')} <b>نتایج قرعه‌کشی اعلام شد!</b>\n\n"
                                    f"{pe('trophy', '🏆')} برندگان ({len(selected)} نفر):{winner_lines}\n\n"
                                    f"{pe('gift', '🎊')} تبریک به برندگان عزیز!"
                                )
                            ...
                            lot["status"] = "done"
                            changed = True
                            print(f"{pe('checkmark_green', '✅')} قرعه‌کشی {lot['id']} پایان یافت")
                        except Exception as e:
                            print(f"{pe('cross_red', '❌')} خطا در پایان قرعه‌کشی: {e}")

                if changed:
                    db.set_global_setting("lotteries", _json.dumps(lotteries, ensure_ascii=False))

            except Exception as e:
                print(f"{pe('cross_red', '❌')} خطا در lottery_scheduler: {e}")
    ...
    def callback_join_lottery(call):
        ...
        for lot in lotteries:
            if lot.get("id") == lot_id and lot.get("status") == "active":
                ...
                if already:
                    return _bot.answer_callback_query(call.id, f"{pe('checkmark_green', '✅')} شما قبلاً ثبت‌نام کرده‌اید!", show_alert=True)
                ...
                try:
                    ...
                    if end_dt.tzinfo is None:
                        end_dt = end_dt.replace(tzinfo=_TEHRAN_OFFSET)
                    if now_teh > end_dt:
                        return _bot.answer_callback_query(call.id, f"{pe('timer_1h', '⏰')} مهلت شرکت تمام شده است.", show_alert=True)
                except Exception:
                    pass
                ...
                db.set_global_setting("lotteries", _json.dumps(lotteries, ensure_ascii=False))
                return _bot.answer_callback_query(call.id,
                    f"{pe('checkmark_green', '✅')} ثبت‌نام شما انجام شد! ({len(participants)} نفر شرکت کرده‌اند)",
                    show_alert=True)

        _bot.answer_callback_query(call.id, f"{pe('cross_red', '❌')} قرعه‌کشی یافت نشد یا پایان یافته.", show_alert=True)
        ...
        while True:
            try:
                ...
                if "409" in str(e):
                    ...
                    print(f"{pe('warning', '⚠️')} خطای polling: {e}")
                    time.sleep(3)

    t = threading.Thread(target=_polling_loop, daemon=True)
    t.start()
    print(f"{pe('checkmark_green', '✅')} ربات الماس @{BOT_USERNAME} استارت شد")
